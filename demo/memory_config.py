...
class EmbeddingConfig:
    ...
    def __post_init__(self):
        """从环境变量加载配置"""
        self.base_url = self.base_url or os.getenv("EMB_BASE_URL") or "http://0.0.0.0:11000/v1/embeddings"
...
class ExtractModeConfig:
    ...
    # 输出目录
    output_dir: Path = field(default_factory=lambda: Path(__file__).parent / "memcell_outputs")
    ...
    max_memcells_per_profile_batch: int = 50  # 单次 Profile 提取最多使用多少个 MemCell（避免超出 LLM token 限制）
    profile_batch_strategy: str = "latest_first"  # 分批策略："latest_first"（最新优先）, "distributed"（均匀分布）, "sequential"（顺序分批）
...
class ChatModeConfig:
    ...
    # 路径配置
    chat_history_dir: Path = field(default_factory=lambda: Path(__file__).parent / "chat_history")
    memcell_output_dir: Path = field(default_factory=lambda: Path(__file__).parent / "memcell_outputs")
