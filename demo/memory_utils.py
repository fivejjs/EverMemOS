...
class PerformanceMetrics:
    ...
    def report(self) -> None:
        ...
        if self.embedding_calls > 0:
            print(f"  - 平均耗时: {self.embedding_total_time/self.embedding_calls:.3f}秒/次")
...
class ProgressTracker:
    ...
    def update(self, n: int = 1) -> None:
        ...
        if current_time - self.last_update > 0.5 or self.current >= self.total:
            ...
            print(f'\r{self.desc}: |{bar}| {self.current}/{self.total} [{pct:.1f}%] '
...
class BatchMongoWriter:
    ...
    def __init__(self, batch_size: int = 20, perf_metrics: Optional[PerformanceMetrics] = None):
        """初始化批量写入器
    ...
    async def _flush(self) -> None:
        ...
        try:
            # 导入文档模型
            from src.infra_layer.adapters.out.persistence.document.memory.memcell import MemCell as DocMemCell
            ...
            print(f"[MongoDB] 批量写入 {len(self.buffer)} 个MemCell，耗时 {elapsed:.2f}秒")
            self.buffer.clear()
...
async def query_memcells_by_group_and_time(
    ...
    Returns:
        MemCell 文档对象列表
    """
    memcells = await DocMemCell.find({
        "group_id": group_id,
...
def load_user_profiles_from_dir(
    output_dir: Path
...
class VectorSimilarityStrategy(RetrievalStrategy):
    ...
    async def retrieve(
        ...
        for candidate in candidates:
            try:
                ...
                print(f"[VectorSimilarity] ⚠️ 嵌入向量获取失败: {getattr(candidate, "episode", "")}")
                doc_episode_vecs.append([0 for _ in range(1024)])
        
        if len(doc_episode_vecs) != len(candidates):
            print(f"[VectorSimilarity] ⚠️ 嵌入向量数量不匹配: {len(doc_episode_vecs)} != {len(candidates)}")
            return []
    ...
    async def retrieve_semantic(
        ...
        date_query_naive = date_query.replace(tzinfo=None) if date_query.tzinfo else date_query
        ...
        for candidate in candidates:
            ...
            for semantic_memory in candidate.semantic_memories:
                ...
                if isinstance(end_time, str):
                    ...
                    end_time_dt = end_time.replace(tzinfo=None) if end_time.tzinfo else end_time
                else:
                    # 不支持的类型，跳过
                    continue
