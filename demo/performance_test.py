...
from memory_layer.types import MemCell, MemoryType, RawDataType
...
class PerformanceTester:
    ...
    async def test_memcell_extraction(self):
        ...
        for idx, new_msg in enumerate(new_messages, 1):
            ...
            if memcell:
                ...
                print(f"        - Event ID: {memcell.event_id}")
                print(
                    f"        - Summary: {memcell.summary[:100] if memcell.summary else 'N/A'}..."
                )
                print(
                    f"        - Subject: {memcell.subject[:100] if memcell.subject else 'N/A'}..."
                )
                print(
                    f"        - Episode: {'有' if memcell.episode else '无'} ({len(memcell.episode) if memcell.episode else 0} 字符)"
                )
                print(
                    f"        - 参与者: {', '.join(memcell.participants) if memcell.participants else 'N/A'}"
                )
                ...
                print(
                    f"        - Semantic Memory: {'有' if hasattr(memcell, 'semantic_memories') and memcell.semantic_memories else '无'}"
                )
                print(
                    f"        - Event Log: {'有' if hasattr(memcell, 'event_log') and memcell.event_log else '无'}"
                )
        ...
        print(f"\n  📊 逐条消息耗时统计:")
        print(
            f"     {'消息序号':<10} {'耗时 (ms)':<15} {'耗时 (秒)':<12} {'状态':<15} {'内容预览':<30}"
        )
        ...
        for timing in message_timings:
            ...
            content = (
                timing["content_preview"] + "..."
                if len(timing["content_preview"]) > 27
                else timing["content_preview"]
            )
            print(
                f"     第{timing['index']:2d}条    {timing['duration']:>12.2f} ms  {timing['duration'] / 1000:>10.2f} 秒  {status:<15} {content:<30}"
            )

        if message_timings:
            avg_duration = sum(t["duration"] for t in message_timings) / len(
                message_timings
            )
            ...
            print(
                f"     - 总耗时: {total_duration:.2f} ms ({total_duration / 1000:.2f} 秒)"
            )
            print(
                f"     - 平均每条消息耗时: {avg_duration:.2f} ms ({avg_duration / 1000:.2f} 秒)"
            )
        ...
        if memcell:
            ...
            print(f"  Event ID: {memcell.event_id}")
            print(
                f"  Episode 长度: {len(memcell.episode) if memcell.episode else 0} 字符"
            )
            print(
                f"  语义记忆: {'有' if hasattr(memcell, 'semantic_memories') and memcell.semantic_memories else '无'}"
            )
            print(
                f"  事件日志: {'有' if hasattr(memcell, 'event_log') and memcell.event_log else '无'}"
            )

            # 分析触发 MemCell 提取的那条消息
            extracted_msg_timing = next(
                (t for t in message_timings if t["memcell_extracted"]), None
            )
            if extracted_msg_timing:
                print(f"\n  🎯 触发提取的消息分析:")
                print(f"     - 触发消息序号: 第 {extracted_msg_timing['index']} 条")
                print(
                    f"     - 触发消息耗时: {extracted_msg_timing['duration']:.2f} ms ({extracted_msg_timing['duration'] / 1000:.2f} 秒)"
                )
                print(
                    f"     - 该消息包含: 边界检测 + Episode提取（语义记忆和事件日志已禁用）"
                )
                ...
                for step_name, step_time in trigger_steps.items():
                    ...
                    print(
                        f"     - {step_name:<35} {step_time:>10.2f} ms ({percentage:>5.2f}%)"
                    )

            return memcell
        else:
            print(
                f"  ❌ MemCell 未提取 (should_wait: {status_result.should_wait if status_result else 'N/A'})"
            )
            print(f"  💡 说明:")
    ...
    async def test_profile_memory_extraction(self, memcell):
        ...
        if profile_memories:
            ...
            for idx, profile in enumerate(profile_memories, 1):
                ...
                print(
                    f"     - Memory Type: {profile.memory_type.value if hasattr(profile, 'memory_type') else 'N/A'}"
                )
                print(
                    f"     - Subject: {profile.subject[:100] if profile.subject else 'N/A'}..."
                )
                print(
                    f"     - Summary: {profile.summary[:200] if profile.summary else 'N/A'}..."
                )
    ...
    async def test_event_log_extraction(self, episode_memory):
        ...
        if event_log:
            ...
            if hasattr(event_log, "events") and event_log.events:
                ...
                # 尝试打印对象的属性
                attrs = {
                    k: v for k, v in event_log.__dict__.items() if not k.startswith("_")
                }
    ...
    def print_summary(self):
        ...
        for result in self.results:
            ...
            percentage = (
                (result.duration_ms / total_time) * 100 if total_time > 0 else 0
            )
            print(
                f"{result.step_name:<45} {result.duration_ms:>12.2f} ms  {percentage:>6.2f}%  {status:<10} {result.details}"
            )
