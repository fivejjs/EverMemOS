...
def split_memcells_into_batches(
    ...
    if strategy == "latest_first":
        # 按时间戳降序排序（最新的在前）
        sorted_memcells.sort(
            key=lambda mc: getattr(mc, 'timestamp', ''),
            reverse=True
        )
...
def load_events(path: Path) -> List[Dict[str, Any]]:
    ...
    if isinstance(data, dict):
        ...
        if conversation_list is not None:
            ...
            raise ValueError(
                "`conversation_list` 字段必须为数组，请检查输入 JSON。"
            )
...
async def extract_individual_profiles_from_group_memcells(
    ...
    if enable_token_estimation:
        ...
        if estimated_tokens > max_estimated_tokens:
            print(f"[ProfileExtract] ⚠️ 预估 Token 数超过限制 ({max_estimated_tokens:,})，强制分批处理")
            # 根据 token 数动态调整批次大小
            if estimated_tokens > 0:  # 防止除零错误
                adjusted_batch_size = int(max_batch_size * (max_estimated_tokens / estimated_tokens))
    ...
    if total_batches == 1:
        ...
        print(f"[ProfileExtract] 📦 将 {len(memcell_list)} 个 MemCell 分为 {total_batches} 批处理")
    ...
    try:
        for batch_idx, batch_memcells in enumerate(batches, 1):
            ...
            if enable_token_estimation:
                ...
                print(f"[ProfileExtract]   📏 批次 Token 预估: ~{batch_tokens:,} tokens")
            ...
            if extract_config.scenario_type == ScenarioType.GROUP_CHAT:
                batch_profile_memories = await profile_extractor.extract_memory(extract_request)
            elif extract_config.scenario_type == ScenarioType.ASSISTANT:
                batch_profile_memories = await profile_extractor.extract_profile_companion(extract_request)
            else:
                raise ValueError(f"Invalid scenario type: {extract_config.scenario_type}")
            ...
            print(f"[ProfileExtract]   ✅ 批次 {batch_idx} 返回 {len(batch_profile_memories)} 个 Profile")
            ...
            print(f"[ProfileExtract]   📊 批次 {batch_idx} 完成，耗时: {batch_elapsed:.2f}s")
...
async def extract_and_dump(
    ...
    mongo_writer = BatchMongoWriter(
        ...
        perf_metrics=PERF_METRICS if extract_config.enable_performance_metrics else None
    )
    ...
    # Profile 批量提取：收集所有 MemCell，最后一次性提取
    memcell_batch: List = []
    ...
    for idx, entry in enumerate(events):
        ...
        try:
            llm_start = time.time()
            memcell, status = await extractor.extract_memcell(request, use_semantic_extraction=extract_config.enable_semantic_extraction)
            llm_elapsed = time.time() - llm_start
        ...
        if should_end:
            ...
            await _save_clustering_snapshot(
                extractor,
                extract_config.output_dir,
                extract_config.group_id
            )
            
            # 添加到 Profile 提取批次（收集所有 MemCell）
            if profile_extractor is not None:
                memcell_batch.append(memcell)
    ...
    # 9. 处理所有 MemCell：为每个参与者提取个人 Profile
    if profile_extractor is not None and memcell_batch:
        print(f"\n[Extract] 使用所有 {len(memcell_batch)} 个 MemCell 提取个人 Profile...")
        profile_count = await extract_individual_profiles_from_group_memcells(
            memcell_list=memcell_batch,
...
async def extract_profiles_only(
    ...
    if extract_config.memcell_source == "file":
        ...
        print(f"[ExtractProfileOnly] ❌ 未知的 MemCell 来源: {extract_config.memcell_source}")
...
async def run_extract_profile_only_mode(
    ...
    # 验证输入目录（file 模式）
    if extract_config.memcell_source == "file" and not extract_config.memcell_input_dir.exists():
        print(f"[ExtractProfileOnly] ❌ MemCell 输入目录不存在: {extract_config.memcell_input_dir}")
        return
