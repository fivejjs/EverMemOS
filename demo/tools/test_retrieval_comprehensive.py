...
class RetrievalTester:
                ...
                if result.get("status") == "ok":
                    ...
                    if len(memories) == 0:
                        if allow_empty:
                            ...
                            info_msg = (
                                f"{test_name}: 允许空结果（耗时 {latency:.2f}ms）"
                            )
                            print(f"  ✅ {info_msg}")
                            empty_result = {
                                "test_name": test_name,
                        ...
                        self.failed_tests += 1
                        warning_msg = (
                            f"{test_name}: 返回 0 条记忆（耗时 {latency:.2f}ms）"
                        )
                    ...
                    print(
                        f"  ✅ {test_name}: 找到 {len(memories)} 条记忆，耗时 {latency:.2f}ms{score_info}"
                    )
    ...
    def print_summary(self):
        ...
        for data_source in self.data_sources:
            ...
            print(
                f"  {data_source}: {success}/{total} 成功，平均返回 {avg_count:.1f} 条记忆"
            )
...
async def main():
    ...
    print(
        f"     当前时间({datetime.now().strftime('%Y-%m-%d')}): {result_current.get('count', 0)} 条"
    )
