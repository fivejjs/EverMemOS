...
class AgenticRetrievalTester:
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                if result.get("status") == "ok":
                    ...
                    if reasoning:
                        print(
                            f"     LLM 判断: {reasoning[:60]}{'...' if len(reasoning) > 60 else ''}"
                        )
                    if refined_queries:
                        print(f"     改进查询: {refined_queries}")
                    print(
                        f"     Round1: {round1_count} 条 → Round2: {round2_count} 条 → 最终: {final_count} 条"
                    )
                    ...
                    if memories:
                        ...
                        for i, mem in enumerate(memories[:3], 1):
                            ...
                            print(
                                f"       [{i}] 分数: {score:.4f} | {content[:50]}{'...' if len(content) > 50 else ''}"
                            )
    ...
    def print_summary(self):
        ...
        if self.successful_tests > 0:
            print(
                f"  多轮检索: {self.multi_round_count}/{self.successful_tests} ({(self.multi_round_count / self.successful_tests * 100):.1f}%)"
            )
            print(
                f"  LLM 判定充分: {self.sufficient_count}/{self.successful_tests} ({(self.sufficient_count / self.successful_tests * 100):.1f}%)"
            )

        # 平均耗时
        successful_results = [
            r for r in self.test_results if r.get("status") == "✅ 成功"
        ]
