...
async def clear_all_memories(verbose: bool = True):
    ...
    try:
            ...
            try:
                ...
                if utility.has_collection(alias_name, using=milvus_collection._using):
                    ...
                    if old_real_name:
                        ...
                        if verbose:
                            print(
                                f"      ✅ {coll_name}: 创建新Collection: {new_coll.name}"
                            )
                        ...
                        if verbose:
                            print(
                                f"      ✅ {coll_name}: 已切换alias '{alias_name}' 到新Collection并删除旧Collection '{old_real_name}'"
                            )
                    else:
                        if verbose:
        ...
        try:
            from component.elasticsearch_client_factory import (
            ...
            for index_name in alias_info.keys():
                ...
                while True:
                    ...
                    deleted_count = (
                        response.get("deleted", 0) if isinstance(response, dict) else 0
                    )
                    total_deleted += deleted_count

                    if verbose and deleted_count > 0:
                        print(
                            f"      - 清空索引 {index_name}: 本轮删除 {deleted_count} 条文档"
                        )
            ...
            if verbose:
                print(
                    f"      ✅ Elasticsearch 已清空（总计删除 {total_deleted} 条文档）"
                )
        except Exception as e:
            if verbose:
