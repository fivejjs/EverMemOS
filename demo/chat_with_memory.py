...
class ChatSession:
    ...
    def __init__(
        ...
        self.conversation_history: List[Tuple[str, str]] = []  # [(user_q, assistant_a), ...]
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Profile 文件直接保存在场景类型目录下（不再按 group_id 分子目录）
            # 例如：demo/memcell_outputs/group_chat/profile_user_101.json
            group_output_dir = self.config.memcell_output_dir
            self.user_profiles = load_user_profiles_from_dir(group_output_dir)
            
            if not self.user_profiles:
                print(f"[{self.texts.get('warning_label')}] {self.texts.get('loading_profiles_warning')}")
                print(f"[{self.texts.get('hint_label')}] {self.texts.get('loading_profiles_hint')}")
            else:
                user_names = [
                    get_user_name_from_profile(p) or uid 
                    for uid, p in self.user_profiles.items()
                ]
                print(f"[{self.texts.get('loading_label')}] {self.texts.get('loading_profiles_success', count=len(self.user_profiles), names=', '.join(user_names))} ✅")
    ...
    async def load_conversation_history(self) -> int:
        ...
        try:
            ...
            history_files = []
            # 查找新格式的文件名
            new_files = list(self.config.chat_history_dir.glob(f"{display_name}_*.json"))
            # 查找旧格式的文件名（为了向后兼容）
            old_files = list(self.config.chat_history_dir.glob(f"{self.group_id}_*.json"))
            # 查找更旧的格式（兼容之前的旧名称）
            very_old_files = list(self.config.chat_history_dir.glob("EverMem 928交付群_*.json"))

            history_files = sorted(new_files + old_files + very_old_files, reverse=True)  # 最新的在前
    ...
    def build_prompt(
        ...
        if profiles:
            if self.scenario_type == ScenarioType.ASSISTANT:
                profile_content = profiles.get("user_001", {}).get("personality")[0].get("analysis")
                # print(profile_content)

                messages.append({
                    "role": "system",
                    "content": self.texts.get(f"prompt_profile_prefix_{lang_key}") + profile_content
                })
                ...
                for user_id, profile in profiles.items():
                    ...
                    profile_list.append(json.dumps(user_profile, ensure_ascii=False, indent=2))
                
                if profile_list != []:
                    profile_content = self.texts.get(f"prompt_profile_prefix_{lang_key}") + "\n".join(profile_list)
                    # print(profile_content)
                    messages.append({
                        "role": "system",
        ...
        if memories:
            ...
            for i, mem in enumerate(memories, start=1):
                ...
                summary = mem.get("summary") or mem.get("episode", "")
                ...
                if summary:
                    # 给 LLM 提供更多信息（不截断），但限制在 500 字符内
                    summary_text = summary[:500] + ("..." if len(summary) > 500 else "")
                    parts.append(self.texts.get("prompt_memory_content", content=summary_text))
    ...
    async def chat(self, user_input: str) -> str:
        ...
        try:
            ...
            if structured_response is None:
                # JSON 解析失败，使用回退方案
                print(f"[{self.texts.get('warning_label')}] LLM 输出非 JSON 格式，使用回退方案")
    ...
    async def reload_data(self) -> None:
        ...
        print()
        ui.success(f"✓ {self.texts.get('cmd_reload_complete', users=len(self.user_profiles), memories=self.memcell_count)}")
        print()
...
class ChatUI:
    ...
    def print_retrieved_memories(
        ...
        if retrieval_method == "vector_similarity":
            method_label = "向量相似度检索" if texts.language == "zh" else "Vector Similarity Retrieval"
        elif retrieval_method == "semantic_memory":
            method_label = "语义记忆检索" if texts.language == "zh" else "Semantic Memory Retrieval"
        else:
            # default 模式
    ...
    def print_response_metadata(response: "StructuredResponse", texts: I18nTexts):
        ...
        refs = f"📎 {texts.get('response_references')}: {', '.join(response.references)}" if response.references else ""
        line = f"{emoji} {texts.get('response_confidence')}: {response.confidence.upper()}"
        if refs:
            line += f"  |  {refs}"
        print()
        ui.text(line)
    ...
    def print_full_reasoning(response: "StructuredResponse", texts: I18nTexts):
        ...
        confidence_emoji = {"high": "🟢", "medium": "🟡", "low": "🔴"}
        emoji = confidence_emoji.get(response.confidence, "⚪")
        refs_text = ', '.join(response.references) if response.references else texts.get("response_no_references")
        meta_lines = [
