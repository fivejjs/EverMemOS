...
def get_default_kafka_config() -> Dict[str, Any]:
    ...
    if os.getenv("CA_FILE_PATH"):
        ca_file_path = get_ca_file_path(os.getenv("CA_FILE_PATH", "kafka/ca_file/AmazonRootCA1.pem"))
...
class KafkaConsumerFactory:
        ...
        **kwargs
        ...
        async with self._lock:
            ...
            if force_new or cache_key not in self._consumers:
                logger.info("Creating new consumer for %s (force_new=%s)", consumer_name, force_new)
                ...
                logger.info("Consumer %s created and cached with key %s", consumer_name, cache_key)
            else:
    ...
    async def seek_to_datetime(
        ...
        try:
            ...
            logger.info("Seeking consumer to datetime: %s (timestamp: %d)", 
                       offset_datetime, target_timestamp_ms)
            ...
            try:
                ...
                if not partitions:
                    raise RuntimeError("Consumer has no assigned partitions. Make sure consumer is started and has subscribed to topics.")
            except Exception as e:
            ...
            for partition in partitions:
                ...
                if offset_info is not None:
                    ...
                    logger.info("Seeked partition %s (topic: %s) to offset %d at timestamp %d", 
                               partition, topic_name, target_offset, target_timestamp_ms)
                else:
                    # 没有找到对应时间戳的offset，使用最新offset
                    logger.warning("No offset found for partition %s (topic: %s) at timestamp %d, using latest offset", 
                                 partition, topic_name, target_timestamp_ms)
                    ...
                    logger.info("Seeked partition %s (topic: %s) to latest offset %d", 
                               partition, topic_name, latest_offset)
            
            # 记录各topic的处理统计
            for topic_name, stats in topic_stats.items():
                logger.info("Topic '%s': %d partitions total, %d found timestamp offsets, %d used latest offsets", 
                           topic_name, stats['total_partitions'], stats['found_offsets'], stats['used_latest'])
            
            if seek_count > 0:
                logger.info("Successfully seeked %d partitions to datetime %s", 
                           seek_count, offset_datetime)
                return True
            else:
            ...
            raise ValueError(f"Invalid datetime format '{offset_datetime}'. Expected format: 'YYYY-MM-DD HH:MM:SS'") from e
