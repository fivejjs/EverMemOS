...
class RedisProvider:
    ...
    async def get_named_client(self, name: str, **overrides) -> redis.Redis:
        ...
        async with self._lock:
            ...
            try:
                ...
                # 创建命名连接池
                named_pool = ConnectionPool.from_url(
                    self.redis_url,
                    **conn_params
                )
                ...
                logger.info("命名Redis客户端初始化成功: %s (参数覆盖: %s)", 
                           name, overrides if overrides else "无")
    ...
    async def set(self, key: str, value: Union[str, bytes, int, float], ex: Optional[int] = None, nx: bool = False) -> bool:
        """
        设置键值对
    ...
    async def expire(self, key: str, seconds: int) -> bool:
        ...
        try:
            ...
            logger.error("Redis EXPIRE操作失败: key=%s, seconds=%s, error=%s", key, seconds, str(e))
            return False
