...
def get_default_es_config() -> Dict[str, Any]:
    ...
    logger.info("  认证: %s", "API Key" if es_api_key else ("Basic" if es_username else "无"))
    
    return config


def get_cache_key(hosts: List[str], username: Optional[str] = None, api_key: Optional[str] = None) -> str:
    """
    生成缓存键
    基于 hosts、认证信息生成唯一标识
...
class ElasticsearchClientWrapper:
    ...
    async def initialize_indices(self, document_classes: Optional[List[Type[DocBase]]] = None):
        """初始化索引"""
        ...
        if document_classes:
            try:
                logger.info("正在初始化 Elasticsearch 索引，共 %d 个文档类", len(document_classes))
                ...
                logger.info("✅ Elasticsearch 索引初始化成功，处理了 %d 个文档类", len(document_classes))
                
                for doc_class in document_classes:
                    logger.info("📋 初始化索引: class=%s -> index=%s", 
                              doc_class.__name__, 
                              doc_class._index._name if hasattr(doc_class, '_index') else 'unknown')
...
class ElasticsearchClientFactory:
    ...
    def create_async_connection(
        ...
        **kwargs
        ...
        logger.info("Created elasticsearch_dsl async connection for %s with alias '%s'", hosts, alias)
    ...
    async def get_client(
        ...
        **kwargs
        ...
        async with self._lock:
            ...
            logger.info("Elasticsearch client %s created and cached with key %s", hosts, cache_key)
