...
class SemanticMemoryExtractor(MemoryExtractor):
    ...
    async def generate_semantic_memories_for_memcell(
        ...
        try:
            ...
            logger.debug(f"✅ LLM调用完成，响应长度: {len(response) if response else 0}")
            
            # 解析JSON响应
            start_time = self._extract_start_time_from_timestamp(memcell.timestamp)
            semantic_memories = await self._parse_semantic_memories_response(response, memcell.event_id, start_time)
            
            # 确保返回恰好10条
            if len(semantic_memories) > 10:
                semantic_memories = semantic_memories[:10]
            elif len(semantic_memories) < 10:
                logger.warning(f"生成的语义记忆联想数量不足10条，实际生成: {len(semantic_memories)}")
    ...
    async def generate_semantic_memories_for_episode(
        ...
        try:
            ...
            # 解析JSON响应
            source_episode_id = episode.ori_event_id_list[0] if episode.ori_event_id_list else None
            start_time = self._extract_start_time_from_timestamp(episode.timestamp)
            semantic_memories = await self._parse_semantic_memories_response(response, source_episode_id, start_time)
    ...
    async def _parse_semantic_memories_response(self, response: str, source_episode_id: Optional[str] = None, start_time: Optional[str] = None) -> List[SemanticMemoryItem]:
        """
