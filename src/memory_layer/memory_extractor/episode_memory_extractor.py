...
class EpisodeMemoryExtractor(MemoryExtractor):
    ...
    async def _trigger_semantic_extraction_async(self, episode_memories: List[EpisodeMemory], request: EpisodeMemoryExtractRequest):
        """
        异步触发语义记忆提取，不影响主流程
        ...
        async def extract_semantic_for_episode(episode_memory: EpisodeMemory):
            ...
            for attempt in range(max_retries):
                try:
                    logger.debug(f"🧠 自动触发语义记忆提取开始: episode_memory='{episode_memory.subject}' (尝试 {attempt + 1}/{max_retries})")
                    semantic_memories = await self.semantic_extractor.generate_semantic_memories_for_episode(episode_memory)
                    episode_memory.semantic_memories = semantic_memories
                    logger.info(f"✅ 为情景记忆 '{episode_memory.subject}' 生成了 {len(semantic_memories)} 条语义记忆")
                    return True  # 成功
                except Exception as e:
                    logger.error(f"❌ 为情景记忆 '{episode_memory.subject}' 生成语义记忆时出错: {e} (尝试 {attempt + 1}/{max_retries})")
                    
                    if attempt < max_retries - 1:
                        await asyncio.sleep(0.5)
                    else:
                        logger.error(f"❌ 所有重试次数均失败，未能为情景记忆 '{episode_memory.subject}' 提取语义记忆")
                        return False  # 失败
            return False
        
        # 并发处理所有episode的语义记忆提取
        tasks = [extract_semantic_for_episode(episode_memory) for episode_memory in episode_memories]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        # 统计结果
        success_count = sum(1 for result in results if result is True)
        logger.info(f"语义记忆提取完成: {success_count}/{len(episode_memories)} 个episode成功")

    async def _trigger_semantic_extraction_for_memcell_async(self, memcell: MemCell, request: EpisodeMemoryExtractRequest):
        """
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"🧠 自动触发语义记忆提取开始: memcell='{memcell.subject}' (尝试 {attempt + 1}/{max_retries})")
                semantic_memories = await self.semantic_extractor.generate_semantic_memories_for_memcell(memcell)
                memcell.semantic_memories = semantic_memories
                logger.info(f"✅ 为MemCell '{memcell.subject}' 生成了 {len(semantic_memories)} 条语义记忆")
                break  # 成功则跳出重试循环
            except Exception as e:
                logger.error(f"❌ 为MemCell '{memcell.subject}' 生成语义记忆时出错: {e} (尝试 {attempt + 1}/{max_retries})")
                
                if attempt < max_retries - 1:
                    await asyncio.sleep(0.5)
                else:
                    logger.error(f"❌ 所有重试次数均失败，未能为MemCell '{memcell.subject}' 提取语义记忆")

    async def extract_memory(self, request: EpisodeMemoryExtractRequest, use_group_prompt: bool = False, use_semantic_extraction: bool = False) -> Optional[List[EpisodeMemory]] | Optional[MemCell]:
        ...
        if use_group_prompt:
            ...
            if use_semantic_extraction:
                await self._trigger_semantic_extraction_for_memcell_async(first_memcell, request)
