        ...
        if isinstance(project_data, ProjectInfo):
            projects.append(
                ProjectInfo(
                    ...
                    subtasks=list(project_data.subtasks)
                    if project_data.subtasks
                    else None,
                    user_objective=list(project_data.user_objective)
                    if project_data.user_objective
                    else None,
                    contributions=list(project_data.contributions)
                    if project_data.contributions
                    else None,
                    user_concerns=list(project_data.user_concerns)
...
def merge_projects_participated(
    ...
    def clone_project(project: ProjectInfo) -> ProjectInfo:
        return ProjectInfo(
            ...
            user_objective=list(project.user_objective)
            if project.user_objective
            else None,
            contributions=list(project.contributions)
            if project.contributions
            else None,
            user_concerns=list(project.user_concerns)
            if project.user_concerns
            else None,
        )
