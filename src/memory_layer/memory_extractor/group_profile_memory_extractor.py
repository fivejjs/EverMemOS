...
from common_utils.datetime_utils import get_now_with_timezone, from_timestamp, from_iso_format, timezone
...
class TopicInfo:
    ...
    @classmethod
    def create_with_id(cls, name: str, summary: str, status: str, last_active_at: datetime, id: Optional[str] = None):
    ...
    async def extract_memory(
        ...
        try:
            ...
            valid_memcell_ids = set(
                str(mc.event_id) for mc in memcell_list 
            ...
            # 构建综合speaker映射
            comprehensive_mapping = self.data_processor.get_comprehensive_speaker_mapping(
            ...
            logger.info(f"[extract_memory] Processed roles with {sum(len(v) for v in all_roles.values())} total assignments")
            ...
            logger.error(
                f"[GroupProfileMemoryExtractor] Extraction error: {e}", 
                exc_info=True
            )
            return None
