...
class GroupProfileLLMHandler:
    ...
    async def execute_parallel_analysis(
        ...
        content_prompt = self.build_content_analysis_prompt(
            conversation_text, group_id, group_name, 
            existing_topics, existing_summary, existing_subject, timespan
        )
        behavior_prompt = self.build_behavior_analysis_prompt(
            conversation_text, group_id, group_name, memcell_list, 
            existing_roles, user_organization
        )
        
        # 并行执行
        logger.info(f"[LLMHandler] Executing parallel analysis for group: {group_name}")
        content_task = self._execute_with_retry("Content Analysis", 
            lambda: self.llm_provider.generate(content_prompt, temperature=0.3))
        behavior_task = self._execute_with_retry("Behavior Analysis",
        ...
        return self._merge_parallel_analysis_results(
            content_response, behavior_response, group_id, group_name, 
            memcell_list, existing_profile
        )
    ...
    def build_behavior_analysis_prompt(
        ...
        speaker_info_title = "**Available Speakers with Organization Context:**" if has_org_info else "**Available Speakers:**"
        speaker_info = f"\n{speaker_info_title}\n"
