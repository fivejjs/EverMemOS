...
class TopicProcessor:
    ...
    def find_topic_to_replace(self, topics: List, reference_time: Optional[datetime] = None) -> object:
        ...
        return sorted(topics, key=lambda t: (
            ...
            not (t.status == "implemented" and 
                 (t.last_active_at or now) < threshold),
            # 第二优先级：按时间从旧到新
    ...
    def get_latest_memcell_timestamp(
        self, 
        memcell_list: List, 
        memcell_ids: Optional[List[str]] = None
    ) -> datetime:
        """
        ...
        if filter_ids and matched_count > 0:
            logger.debug(f"[get_latest_memcell_timestamp] Found {matched_count} memcells matching {len(filter_ids)} IDs")
