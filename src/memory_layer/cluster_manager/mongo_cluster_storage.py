...
class MongoClusterStorage(ClusterStorage):
    ...
    async def save_cluster_state(self, group_id: str, state: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to save cluster state for group {group_id}: {e}", exc_info=True
            )
            return False
    ...
    async def load_cluster_state(self, group_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            cluster_state = await ClusterState.find_one(
                ClusterState.group_id == group_id
            )
            ...
            logger.error(
                f"Failed to load cluster state for group {group_id}: {e}", exc_info=True
            )
            return None
    ...
    async def get_cluster_assignments(self, group_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get cluster assignments for group {group_id}: {e}",
                exc_info=True,
            )
            return {}
    ...
    async def clear(self, group_id: Optional[str] = None) -> bool:
        ...
        try:
            if group_id is None:
                ...
                logger.info(
                    f"Cleared all cluster states, deleted {result.deleted_count} documents"
                )
            else:
                # 清除指定群组的聚类状态
                cluster_state = await ClusterState.find_one(
