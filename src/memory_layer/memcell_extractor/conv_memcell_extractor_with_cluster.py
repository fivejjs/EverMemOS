...
from ..memory_extractor.episode_memory_extractor import EpisodeMemoryExtractor, EpisodeMemoryExtractRequest
...
class _GroupClusterState:
    ...
    def from_dict(data: Dict[str, Any]) -> "_GroupClusterState":
        ...
        st.cluster_centroids = {k: np.array(v, dtype=np.float32) for k, v in centroids.items()}
        st.cluster_counts = {k: int(v) for k, v in (data.get("cluster_counts", {}) or {}).items()}
        st.cluster_last_ts = {k: float(v) for k, v in (data.get("cluster_last_ts", {}) or {}).items()}
        return st

    def _add_vector_to_cluster(self, cid: str, v: np.ndarray, ts: Optional[float]) -> None:
        ...
        if state.cluster_centroids:
            ...
            for ccid, centroid in state.cluster_centroids.items():
                ...
                if ts is not None:
                    ...
                    if last_ts is not None and abs(ts - (last_ts or 0.0)) > self.params.max_link_gap_seconds:
                        continue
    ...
    def dump_to_dir(self, output_dir: str) -> None:
        ...
        for gid, state in self._states.items():
            ...
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"assignments": state.eventid_to_cluster}, f, ensure_ascii=False, indent=2)
    ...
    def load_state_from_dir(self, input_dir: str) -> None:
        try:
            for fname in os.listdir(input_dir):
                if not fname.startswith("cluster_state_") or not fname.endswith(".json"):
                    continue
...
class ConvMemCellExtractorWithCluster(MemCellExtractor):
    def __init__(self, llm_provider=LLMProvider, embedding_base_url: Optional[str] = None, embedding_model: Optional[str] = None, **llm_kwargs):
    ...
    async def extract_memcell(self, request: ConversationMemCellExtractRequest, use_semantic_extraction: bool = False) -> tuple[Optional[MemCell], Optional[StatusResult]]:
        ...
        if should_end:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    now = time.time()
                    episode_result = await self.episode_extractor.extract_memory(episode_request, use_group_prompt=True, use_semantic_extraction=use_semantic_extraction)
                    ...
                    if episode_result and isinstance(episode_result, MemCell):
                        ...
                        try:
                            ...
                            if text_for_embed:
                                ...
                                episode_result.extend["embedding"] = vec.tolist() if hasattr(vec, "tolist") else list(vec)
                        ...
                        try:
                            self._cluster_worker.submit(request.group_id, episode_result.to_dict())
                        except Exception:
            ...
            try:
                ...
                if text_for_embed:
                    ...
                    memcell.extend["embedding"] = vec.tolist() if hasattr(vec, "tolist") else list(vec)
