...
class MongoProfileStorage(ProfileStorage):
    ...
    async def save_profile(
        ...
        try:
            ...
            if existing:
                ...
                await existing.save()
                logger.info(
                    f"Updated profile for user {user_id} in group {group_id} (version {existing.version})"
                )
            else:
                # 创建新文档
                user_profile = UserProfile(
                    ...
                    version=1,
                    cluster_ids=[metadata["cluster_id"]]
                    if "cluster_id" in metadata
                    else [],
            ...
            logger.error(
                f"Failed to save profile for user {user_id}: {e}", exc_info=True
            )
            return False
    ...
    async def get_profile(
        ...
        try:
            ...
            logger.error(
                f"Failed to get profile for user {user_id}: {e}", exc_info=True
            )
            return None
    ...
    async def get_all_profiles(self, group_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get all profiles for group {group_id}: {e}", exc_info=True
            )
            return {}
    ...
    async def get_profile_history(
        ...
        try:
            ...
            logger.error(
                f"Failed to get profile history for user {user_id}: {e}", exc_info=True
            )
            return []
    ...
    async def clear(self, group_id: Optional[str] = None) -> bool:
        ...
        try:
            if group_id is None:
                ...
                logger.info(
                    f"Cleared all user profiles, deleted {result.deleted_count} documents"
                )
            else:
                # 清除指定群组的用户画像
                result = await UserProfile.find(
                    UserProfile.group_id == group_id
                ).delete()
