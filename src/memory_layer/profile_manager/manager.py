...
from memory_layer.profile_manager.storage import InMemoryProfileStorage, ProfileStorage
...
class ProfileManager:
    ...
    def __init__(
        ...
        scenario_str = (
            ...
            if isinstance(self.config.scenario, ScenarioType)
            else str(self.config.scenario)
        )
        ...
        self._watched_clusters: Set[str] = (
            set()
        )  # clusters flagged for profile extraction
    ...
    async def on_memcell_clustered(
        ...
        print(
            f"[ProfileManager] 已添加 MemCell 到 _cluster_memcells[{cluster_id}]，当前数量: {len(self._cluster_memcells[cluster_id])}"
        )
        ...
        context = (
            recent_memcells
            if recent_memcells is not None
            else self._recent_memcells[-2:]
        )
        ...
        logger.info(
            f"is_high_value: {is_high_value}, confidence: {confidence}, reason: {reason}"
        )
        ...
        if cluster_id in self._watched_clusters and self.config.auto_extract:
            ...
            print(
                f"[ProfileManager] Cluster {cluster_id}: {cluster_memcell_count} MemCells, 阈值: {min_memcells_for_extraction}"
            )
            ...
            if cluster_memcell_count < min_memcells_for_extraction:
                ...
                try:
                    print(
                        f"[ProfileManager] 🚀 开始提取 Cluster {cluster_id} 的 Profile..."
                    )
                    updated_profiles = await self._extract_profiles_for_cluster(
                        cluster_id=cluster_id, user_id_list=user_id_list
                    )
                    ...
                    error_msg = (
                        f"Failed to extract profiles for cluster {cluster_id}: {e}"
                    )
    ...
    async def _extract_profiles_for_cluster(
        ...
        print(
            f"[ProfileManager]   _cluster_memcells keys: {list(self._cluster_memcells.keys())}"
        )
        print(
            f"[ProfileManager]   _cluster_memcells[{cluster_id}]: {len(self._cluster_memcells.get(cluster_id, []))} 个"
        )

        memcells = self._cluster_memcells.get(cluster_id, [])
        if not memcells:
            print(
                f"[ProfileManager] ❌ _cluster_memcells[{cluster_id}] 为空！返回空列表"
            )
            return []
        ...
        from infra_layer.adapters.out.persistence.document.memory.memcell import (
            MemCell as MemCellDoc,
        )

        full_memcells = []
        for mc in memcells:
            event_id = (
                getattr(mc, "event_id", None)
                or getattr(mc, "_id", None)
                or getattr(mc, "id", None)
            )
            if event_id:
                print(
                    f"[ProfileManager] 正在从 MongoDB 加载 MemCell: {event_id} (类型: {type(event_id).__name__})"
                )
                ...
                if full_mc:
                    ...
                    print(
                        f"[ProfileManager] ✅ 加载成功，包含 episode: {len(full_mc.episode) if full_mc.episode else 0} 字符"
                    )
                else:
                    print(
                        f"[ProfileManager] ⚠️  未找到 MemCell: {event_id}，使用原始字典"
                    )
                    # 如果找不到，使用原始的字典对象
        ...
        print(
            f"[ProfileManager] ✅ 加载了 {len(full_memcells)} 个完整的 MemCell，开始提取..."
        )
        ...
        for attempt in range(self.config.max_retries):
            try:
                print(
                    f"[ProfileManager] 开始调用 LLM 提取 Profile (场景: {self.config.scenario.value})..."
                )
                ...
                print(
                    f"[ProfileManager] 结果类型: {type(result)}, 长度: {len(result) if result else 0}"
                )

                if not result:
                    logger.warning(
                        f"Profile extraction returned empty result for cluster {cluster_id}"
                    )
                ...
                for i, profile in enumerate(result):
                    ...
                    print(
                        f"[ProfileManager] Profile {i + 1}/{len(result)}: user_id={user_id}, 类型={type(profile).__name__}"
                    )
                        ...
                        save_tasks.append(
                            (
                                ...
                                self._storage.save_profile(
                                    user_id=user_id, profile=profile, metadata=metadata
                                ),
                            )
                        )
                    else:
                        print(
                            f"[ProfileManager] ⚠️  Profile {i + 1} 没有 user_id，跳过保存"
                        )
                ...
                if save_tasks:
                    save_results = await asyncio.gather(
                        *[task[2] for task in save_tasks], return_exceptions=True
                    )
                    ...
                    for (user_id, profile, _), success in zip(save_tasks, save_results):
                        if isinstance(success, Exception):
                            error_msg = (
                                f"Failed to save profile for user {user_id}: {success}"
                            )
                            ...
                            print(
                                f"[ProfileManager] ✅ 成功保存 Profile: user_id={user_id}"
                            )
                        else:
                            logger.warning(f"Failed to save profile for user {user_id}")
                            print(
                                f"[ProfileManager] ❌ 保存失败（返回 False）: user_id={user_id}"
                            )

                print(
                    f"[ProfileManager] 最终成功保存了 {len(updated_profiles)} 个 Profile"
                )
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    logger.error(
                        f"All profile extraction attempts failed for cluster {cluster_id}"
                    )
                    raise
    ...
    def attach_to_cluster_manager(self, cluster_manager: Any) -> None:
        ...
        async def on_cluster_callback(
            group_id: str, memcell: Dict[str, Any], cluster_id: str
        ):
            """Callback for cluster assignment events."""
            print(
                f"[ProfileManager] 🔔 收到聚类回调: cluster_id={cluster_id}, group_id={group_id}"
            )
    ...
    def attach_to_extractor(self, memcell_extractor: Any) -> None:
        ...
        logger.info(
            "ProfileManager successfully attached to MemCellExtractor (legacy mode)"
        )

    async def export_profiles(
        self, output_dir: Path, include_history: bool = True
    ) -> int:
        ...
        for user_id, profile in profiles.items():
            try:
                ...
                with open(latest_file, "w", encoding="utf-8") as f:
                    json.dump(
                        profile_dict, f, ensure_ascii=False, indent=2, default=str
                    )
                    ...
                    if history:
                        ...
                        for i, entry in enumerate(history):
                            ...
                            with open(version_file, "w", encoding="utf-8") as f:
                                json.dump(
                                    entry, f, ensure_ascii=False, indent=2, default=str
                                )
