...
class MsgGroupQueueManager:
    ...
    def __init__(
        ...
        self._queue_stats: List[QueueStats] = [
            QueueStats(queue_id=i, current_size=0) 
        ...
        self._delivery_events: List[deque] = [deque() for _ in range(num_queues)]  # 每个队列的投递事件
        self._consume_events: List[deque] = [deque() for _ in range(num_queues)]   # 每个队列的消费事件
        ...
        logger.info(
            ...
            self.name, self.num_queues, self.max_total_messages
        )
        ...
        try:
            ...
            if not can_deliver:
                ...
                logger.warning(
                    ...
                    self.name, group_key, reject_reason
                )
                return False
            ...
            logger.debug(
                ...
                self.name, group_key, target_queue_id, target_queue.qsize(), self._get_total_current_messages()
            )
            ...
            logger.error(
                ...
                self.name, group_key, e
            )
            return False
    
    async def get_by_queue(self, queue_id: int, wait: bool = True, timeout: Optional[float] = None) -> Optional[Tuple[str, Any]]:
        """
        从指定队列获取消息
        ...
        if queue_id < 0 or queue_id >= self.num_queues:
            raise ValueError(f"队列编号超出范围: {queue_id}, 有效范围: 0-{self.num_queues-1}")
        ...
        try:
            ...
            logger.debug(
                ...
                self.name, queue_id, group_key, target_queue.qsize()
            )
            ...
            logger.debug(
                ...
                self.name, queue_id, timeout
            )
            return None
        except (OSError, ValueError, RuntimeError) as e:
            logger.error(
                "❌ MsgGroupQueueManager[%s] 获取消息失败: queue_id=%d, 错误=%s",
                self.name, queue_id, e
            )
            return None
    ...
    def _can_deliver_message(self) -> Tuple[bool, str]:
        ...
        if current_total >= self.max_total_messages and not has_empty_queue:
            return False, f"当前总消息数={current_total}, 限制={self.max_total_messages}, 无空队列"
    ...
    async def _periodic_log_worker(self):
        ...
        try:
            ...
            logger.error("📊 MsgGroupQueueManager[%s] 定期日志任务异常: %s", self.name, e)
        ...
        try:
            ...
            for queue_info in queue_infos:
                ...
                if is_active:
                    ...
                    # 格式化时间显示
                    last_deliver_display = last_deliver_time[-8:] if last_deliver_time else "无"
                    last_consume_display = last_consume_time[-8:] if last_consume_time else "无"
            ...
            # 打印空闲队列信息
            if idle_queues:
                logger.info("💤 空闲队列: %s (共%d个)", 
                          ", ".join([f"队列[{qid}]" for qid in idle_queues]), 
                          len(idle_queues))
            
            # 打印空队列信息（当前无消息的队列）
            if empty_queues:
                logger.info("📭 空队列: %s (共%d个，可接受新消息)", 
            ...
            if high_backlog_queues:
                logger.warning(
                    "⚠️ 队列积压警告: %s",
                    ", ".join([f"队列[{q['id']}]({q['current']}条)" for q in high_backlog_queues])
                )
                
        except (OSError, ValueError, RuntimeError) as e:
            logger.error("📊 MsgGroupQueueManager[%s] 打印队列详情失败: %s", self.name, e)
    
    async def shutdown(self, mode: ShutdownMode = ShutdownMode.HARD, max_delay_seconds: Optional[float] = None) -> bool:
        """
        关闭管理器，支持硬性和软性关闭模式
        ...
        if mode == ShutdownMode.SOFT:
            ...
            # 检查是否超过延迟时间
            if (self._shutdown_state.max_delay_seconds is not None and 
                current_time - self._shutdown_state.first_soft_shutdown_time >= self._shutdown_state.max_delay_seconds):
                # 超过延迟时间，强制关闭
                logger.warning(
                    "⏰ MsgGroupQueueManager[%s] 软性关闭超时，强制关闭。剩余消息: %d 条",
                    self.name, total_remaining
                )
                await self._perform_hard_shutdown()
                self._shutdown_state.reset()
                return True
            ...
            logger.info(
                ...
                self.name, total_remaining, elapsed_time
            )
            return False
    ...
    async def _perform_hard_shutdown(self):
        ...
        if total_remaining > 0:
            logger.warning(
                ...
                self.name, total_remaining, ", ".join(queue_details)
            )
        else:
            logger.info(
                "🔌 MsgGroupQueueManager[%s] 已安全关闭，无未处理消息",
                self.name
            )
