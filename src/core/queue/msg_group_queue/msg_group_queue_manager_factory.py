...
class MsgGroupQueueManagerFactory:
    ...
    async def get_manager(
        self,
        config: Optional[MsgGroupQueueConfig] = None,
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.error("❌ 创建 MsgGroupQueueManager 失败: %s, 错误: %s", config, e)
                raise
    ...
    async def _get_default_config(self) -> MsgGroupQueueConfig:
        ...
        if self._default_config is None:
            ...
            logger.info("📋 加载默认 MsgGroupQueueManager 配置: %s", self._default_config)
