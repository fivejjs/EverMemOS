...
from core.queue.redis_group_queue.redis_group_queue_item import RedisGroupQueueItem, SerializationMode
from core.queue.redis_group_queue.redis_group_queue_lua_scripts import (
    ENQUEUE_SCRIPT, GET_QUEUE_STATS_SCRIPT, GET_ALL_PARTITIONS_STATS_SCRIPT,
    REBALANCE_PARTITIONS_SCRIPT, JOIN_CONSUMER_SCRIPT, EXIT_CONSUMER_SCRIPT,
    KEEPALIVE_CONSUMER_SCRIPT, CLEANUP_INACTIVE_OWNERS_SCRIPT, FORCE_CLEANUP_SCRIPT,
    GET_MESSAGES_SCRIPT
)
...
class RedisGroupQueueManager:
    ...
    def __init__(
        ...
        self.queue_key_pattern = f"{key_prefix}:queue:{{partition}}"  # partition为001-050
        self.owner_activate_time_zset_key = f"{key_prefix}:owner_activate_time_zset"  # owner活跃时间zset
        ...
        logger.info(
            ...
            self.key_prefix, self.key_prefix, self.max_total_messages
        )
    ...
    def _parse_rebalance_result(self, result: Any, expected_count: int) -> Tuple[bool, Tuple]:
        ...
        if not isinstance(result, (list, tuple)) or len(result) < expected_count:
            logger.error(
                ...
                self.key_prefix, expected_count, result
            )
        ...
        if expected_count == 2:
            ...
            parsed_result = (owner_count, self._convert_flat_to_dict(assigned_partitions_flat))
        elif expected_count == 3:
            cleaned_count, owner_count, assigned_partitions_flat = result
            parsed_result = (cleaned_count, owner_count, self._convert_flat_to_dict(assigned_partitions_flat))
        else:
            return False, tuple([0] * expected_count)
    ...
    async def _check_and_keepalive_if_needed(self, owner_id: str) -> bool:
        ...
        if last_keepalive_time == 0 or (current_time_ms - last_keepalive_time) > 30000:  # 30秒 = 30000毫秒
            logger.debug(
                "💓 RedisGroupQueueManager[%s] 按需触发keepalive: owner_id=%s, 距离上次=%.1f秒",
                self.key_prefix, owner_id, (current_time_ms - last_keepalive_time) / 1000.0
            )
            # 触发keepalive并更新时间戳
            try:
    ...
    async def deliver_message(self, group_key: str, item: RedisGroupQueueItem, return_mode: str = "normal", max_total_messages: int = None) -> bool:
        """
        投递消息到指定分组队列
        ...
        try:
            ...
            if success:
                ...
                logger.debug(
                    ...
                    self.key_prefix, group_key, partition, sort_score, new_count
                )
                ...
                logger.warning(
                    ...
                    self.key_prefix, group_key, partition, message_str
                )
                if return_mode == "normal":
                    return False
                else:
            ...
            try:
                ...
                logger.error(
                    ...
                    self.key_prefix, group_key, partition, e
                )
            except (redis.RedisError, ValueError, TypeError):
                logger.error(
                    "❌ RedisGroupQueueManager[%s] 投递消息失败: group_key=%s, 错误=%s",
                    self.key_prefix, group_key, e
                )
            if return_mode == "normal":
                return False
            else:
        ...
        try:
            ...
            if status_str == "JOIN_REQUIRED":
                ...
                if _retry_depth <= 0:
                    ...
                    raise RuntimeError(f"JOIN_REQUIRED重试次数耗尽: owner_id={owner_id}")
                
                logger.info(
                    "🔄 RedisGroupQueueManager[%s] 需要加入消费者: owner_id=%s, 剩余重试次数=%d",
                    self.key_prefix, owner_id, _retry_depth - 1
                )
                # 自动加入消费者
            ...
            if messages:
                ...
                logger.debug(
                    ...
                    self.key_prefix, owner_id, len(messages)
                )
            else:
                logger.debug(
    ...
    async def rebalance_partitions(self) -> Tuple[int, Dict[str, List[str]]]:
        ...
        try:
            ...
            logger.info(
                ...
                self.key_prefix, owner_count, assigned_partitions
            )
    ...
    async def join_consumer(self, owner_id: Optional[str] = None) -> Tuple[int, Dict[str, List[str]]]:
        ...
        try:
            ...
            logger.info(
                ...
                self.key_prefix, owner_id, owner_count, assigned_partitions
            )
    ...
    @rate_limit(max_rate=1, time_period=2, key_func=lambda owner_id: f"keepalive_consumer_{owner_id}")
    ...
    async def cleanup_inactive_owners(self) -> Tuple[int, int, Dict[str, List[str]]]:
        ...
        try:
            ...
            current_time = int(time.time() * 1000)  # 毫秒时间戳
            inactive_threshold = current_time - (self.inactive_threshold_seconds * 1000)  # 转换为毫秒
            ...
            if cleaned_count > 0:
                logger.info(
                    ...
                    self.key_prefix, cleaned_count, owner_count, assigned_partitions
                )
            else:
                logger.debug(
        ...
        try:
            ...
            if purge_all:
                ...
                logger.warning(
                    ...
                    self.key_prefix, purged_partitions
                )
    ...
    async def get_stats(self, 
                       ...
                       include_consumer_info: bool = False) -> Dict[str, Any]:
        """
        获取统计信息（统一接口）
        ...
        try:
            ...
            total_count, total_messages_in_queues, global_min_score, global_max_score, partition_stats_raw = result
    ...
    @rate_limit(max_rate=1, time_period=5, key_func=lambda group_key: f"get_queue_stats_{group_key}")
    ...
    async def start(self):
        ...
        if self._state == ManagerState.STARTED:
            logger.warning("⚠️ RedisGroupQueueManager[%s] 已经启动，忽略重复启动请求", self.key_prefix)
            return
        
        if self._state == ManagerState.SHUTDOWN:
            raise RuntimeError(f"RedisGroupQueueManager[{self.key_prefix}] 已关闭，不能重新启动")
        
        # 状态必须是 CREATED
        if self._state != ManagerState.CREATED:
            raise RuntimeError(f"RedisGroupQueueManager[{self.key_prefix}] 状态异常: {self._state}")
    ...
    async def start_periodic_tasks(self):
        ...
        try:
            ...
            logger.warning("⚠️ RedisGroupQueueManager[%s] 启动时清理失败: %s", self.key_prefix, e)

        # 启动时立即执行一次log
        try:
            await self._log_manager_details()
            logger.info("🔥 RedisGroupQueueManager[%s] 启动时日志打印完成", self.key_prefix)
        except (redis.RedisError, ValueError, TypeError) as e:
            logger.warning("⚠️ RedisGroupQueueManager[%s] 启动时日志打印失败: %s", self.key_prefix, e)
    ...
    async def _periodic_log_worker(self):
        ...
        try:
            ...
            logger.debug("📊 RedisGroupQueueManager[%s] 定期日志任务被取消", self.key_prefix)
        except (redis.RedisError, ValueError, TypeError) as e:
            logger.error("📊 RedisGroupQueueManager[%s] 定期日志任务异常: %s", self.key_prefix, e)
    ...
    async def _periodic_cleanup_worker(self):
        ...
        try:
            ...
            logger.debug("🧹 RedisGroupQueueManager[%s] 定期清理任务被取消", self.key_prefix)
        except (redis.RedisError, ValueError, TypeError) as e:
            logger.error("🧹 RedisGroupQueueManager[%s] 定期清理任务异常: %s", self.key_prefix, e)
    ...
    async def _log_manager_details(self):
        ...
        try:
            ...
            logger.error("📊 RedisGroupQueueManager[%s] 打印详情失败: %s", self.key_prefix, e)
    ...
    async def shutdown(self, mode: ShutdownMode = ShutdownMode.HARD) -> bool:
        ...
        """
        if self._state == ManagerState.SHUTDOWN:
            logger.warning("⚠️ RedisGroupQueueManager[%s] 已经关闭，忽略重复关闭请求", self.key_prefix)
            return True
        
        if self._state == ManagerState.CREATED:
            logger.info("ℹ️ RedisGroupQueueManager[%s] 未启动状态下关闭", self.key_prefix)
            self._state = ManagerState.SHUTDOWN
            return True
        
        # 状态必须是 STARTED
        if self._state != ManagerState.STARTED:
            logger.warning("⚠️ RedisGroupQueueManager[%s] 状态异常，强制关闭: %s", self.key_prefix, self._state)
        ...
        if mode == ShutdownMode.SOFT:
            ...
            if remaining_messages > 0:
                logger.warning(
                    ...
                    self.key_prefix, remaining_messages
                )
                # 软性关闭失败，但不改变状态，允许重试
                return False
        ...
        try:
            await self._log_manager_details()
            logger.info("🔥 RedisGroupQueueManager[%s] 关闭前最终状态日志完成", self.key_prefix)
        except (redis.RedisError, ValueError, TypeError) as e:
            logger.warning("⚠️ RedisGroupQueueManager[%s] 关闭前日志打印失败: %s", self.key_prefix, e)
