...
class KafkaConsumerRecordItem(RedisGroupQueueItem):
    ...
    def __init__(self, consumer_record: ConsumerRecord):
        ...
        self.key = self._encode_bytes_to_base64(consumer_record.key) if isinstance(consumer_record.key, bytes) else consumer_record.key
        ...
        self.headers = [(name, self._encode_bytes_to_base64(data) if isinstance(data, bytes) else str(data)) 
    ...
    def to_json_str(self) -> str:
        ...
        """
        raise NotImplementedError("KafkaConsumerRecordItem 不支持 JSON 序列化，请使用 to_bson_bytes() 方法")
    ...
    def from_json_str(cls, json_str: str) -> 'KafkaConsumerRecordItem':
        ...
        """
        raise NotImplementedError("KafkaConsumerRecordItem 不支持 JSON 反序列化，请使用 from_bson_bytes() 方法")
...
def consumer_record_to_queue_item(consumer_record: ConsumerRecord) -> KafkaConsumerRecordItem:
    """
...
def queue_item_to_consumer_record(queue_item: KafkaConsumerRecordItem) -> ConsumerRecord:
    """
