    ...
    def _get_owner_id(self) -> str:
        ...
        try:
            ...
            if current_task is None:
                raise DistributedLockError("分布式锁必须在协程环境中使用，当前没有运行的协程任务")
        ...
        for attempt in range(retry_count):
            try:
                ...
                result = await self._lua_acquire(
                    keys=[lock_key],
                    args=[owner_id, timeout_ms],
                    client=redis_client
                )
                
                if result > 0:
                    logger.debug("成功获取可重入锁: %s, 协程: %s, 重入计数: %s (attempt %s)", 
                               resource, owner_id, result, attempt + 1)
                    return True
                else:
                ...
                logger.debug("获取锁失败 (attempt %s): %s, error: %s", 
                           attempt + 1, resource, e)
                if attempt < retry_count - 1:
                    await asyncio.sleep(DEFAULT_RETRY_INTERVAL)
    ...
    async def _release_lock(self, resource: str):
        ...
        try:
            ...
            if result == -1:
                ...
                logger.debug("减少可重入锁计数: %s, 协程: %s, 剩余计数: %s",
                           resource, owner_id, result)
            else:
                logger.warning("无法释放不属于当前协程的锁或者锁不存在: %s, 协程: %s",
                              resource, owner_id)
                
        except (ConnectionError, TimeoutError, OSError) as e:
            logger.error("释放可重入锁时发生异常: %s, 协程: %s, error: %s", 
                       resource, owner_id, e)
