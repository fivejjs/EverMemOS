    ...
    async def startup(self, app: FastAPI) -> Any:
        ...
        try:
            ...
            all_collection_classes = [
                cls for cls in get_all_subclasses(MilvusCollectionBase)
            ...
            for collection_class in all_collection_classes:
                ...
                logger.info("发现 Collection 类: %s [using=%s]", collection_class.__name__, using)
            ...
            for using, collection_classes in using_collections.items():
                ...
                for collection_class in collection_classes:
                    try:
                        ...
                        logger.info("✅ Collection '%s' 初始化成功 [using=%s]", 
                                  collection.name, using)
                    except Exception as e:
                        logger.error("❌ Collection '%s' 初始化失败 [using=%s]: %s", 
                                   collection_class._COLLECTION_NAME, using, e)
                        raise
