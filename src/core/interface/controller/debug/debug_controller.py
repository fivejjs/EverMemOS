...
class BeanCallRequest(BaseModel):
    ...
    # 代码执行方式（可选）
    code: Optional[str] = Field(None, description="Python代码，用于生成args和kwargs参数（可选）")
    
    @classmethod
    def model_validate(cls, obj, *, strict=None, from_attributes=None, context=None):
        """自定义验证逻辑"""
        if isinstance(obj, dict) and not obj.get("bean_name") and not obj.get("bean_type"):
...
class BeanCallResponse(BaseModel):
    ...
    result_str: Optional[str] = Field(None, description="方法返回值的字符串表示（当无法JSON序列化时）")
    ...
    code_execution: Optional[Dict[str, Any]] = Field(None, description="代码执行信息（仅在使用代码生成参数时）")
    ...
    **主要功能**：
    ...
    def _ensure_debug_enabled(self):
        ...
        if not self._check_debug_enabled():
            ...
            raise HTTPException(status_code=404, detail=ErrorMessage.PERMISSION_DENIED.value)
    ...
    def _get_bean_by_identifier(self, bean_name: Optional[str], bean_type: Optional[str]) -> tuple[Any, Dict[str, Any]]:
        """
        根据标识符获取Bean实例和信息
        ...
        if bean_name and bean_type:
            ...
            raise HTTPException(
                status_code=400, 
                detail=ErrorMessage.INVALID_PARAMETER.value
            )
        ...
        try:
            if bean_name:
                ...
                if not bean_class:
                    ...
                    raise HTTPException(
                        status_code=404, 
                        detail=ErrorMessage.BEAN_NOT_FOUND.value
                    )
            ...
            if "not found" in str(e).lower():
                ...
                raise HTTPException(status_code=500, detail=ErrorMessage.BEAN_OPERATION_FAILED.value) from e
    ...
    def _execute_parameter_code(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(args, (list, tuple)):
                logger.error(f"args参数类型错误：期望list或tuple，实际为{type(args).__name__}")
                raise ValueError(ErrorMessage.INVALID_PARAMETER.value)
            
            if not isinstance(kwargs, dict):
                logger.error(f"kwargs参数类型错误：期望dict，实际为{type(kwargs).__name__}")
                raise ValueError(ErrorMessage.INVALID_PARAMETER.value)
    ...
    @get("/beans", 
         ...
         responses={
             ...
             404: {
                 "description": "调试功能未启用或未找到Bean"
             },
             500: {
                 "description": "获取Bean列表时发生内部错误"
             }
         })
        ...
        try:
            ...
            for bean_info in all_beans:
                try:
                    ...
                    beans_info.append(BeanInfoResponse(
                        name=bean_info['name'],
                    ...
                    logger.warning("获取Bean '%s' 的方法列表失败: %s", bean_info['name'], str(e))
                    # 即使获取方法列表失败，也返回基本信息
                    beans_info.append(BeanInfoResponse(
                        name=bean_info['name'],
                        type_name=bean_info['type_name'],
    ...
    @post("/call", 
          ...
          responses={
              ...
              400: {
                  "description": "请求参数错误，如缺少必要参数、Bean标识符无效等"
              },
              404: {
                  "description": "调试功能未启用、Bean不存在或方法不存在"
              },
              500: {
                  "description": "方法调用过程中发生内部错误"
              }
          })
        ...
        **传统方式示例**：
        ...
        try:
            ...
            if not hasattr(bean_instance, request.method):
                logger.error(f"Bean '{bean_info['name']}' 不存在方法 '{request.method}'")
                raise HTTPException(
                    status_code=404, 
                    detail=ErrorMessage.BEAN_OPERATION_FAILED.value
                )
            ...
            if not callable(method_to_call):
                logger.error(f"Bean '{bean_info['name']}' 的属性 '{request.method}' 不是可调用对象")
                raise HTTPException(
                    status_code=400, 
                    detail=ErrorMessage.INVALID_PARAMETER.value
                )
            ...
            # 调用方法
            logger.info("调用Bean方法: %s.%s(args=%s, kwargs=%s)", bean_info['name'], request.method, args, kwargs)
            
            # 检查是否为协程函数，兼容异步和同步方法
            if asyncio.iscoroutinefunction(method_to_call) or inspect.iscoroutinefunction(method_to_call):
    ...
    @get("/beans/{bean_name}", 
         ...
         responses={
             ...
             404: {
                 "description": "调试功能未启用或指定的Bean不存在"
             },
             500: {
                 "description": "获取Bean信息时发生内部错误"
             }
         })
    def get_bean_info(self, bean_name: str) -> BeanInfoResponse:
        ...
        **注意事项**：
        ...
        try:
            ...
            if not bean_meta:
                raise HTTPException(status_code=404, detail=ErrorMessage.BEAN_NOT_FOUND.value)
    ...
    async def call_bean_method_with_code(self, request: BeanCallWithCodeRequest) -> BeanCallResponse:
        """
        ...
        **安全限制**:
        ...
        try:
            # 执行代码生成参数
            logger.info("执行参数生成代码: %s", request.code[:100] + "..." if len(request.code) > 100 else request.code)
