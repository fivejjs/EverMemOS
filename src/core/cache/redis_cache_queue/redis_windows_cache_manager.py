...
class RedisWindowsCacheFactory:
    ...
    async def create_cache_manager(self,
                                 ...
                                 cleanup_probability: float = DEFAULT_CLEANUP_PROBABILITY) -> 'RedisWindowsCacheManager':
        """
        创建缓存管理器实例
...
class RedisWindowsCacheManager:
    ...
    def __init__(self, 
        ...
        logger.info("Redis 时间窗口缓存管理器初始化完成: expire=%d分钟, cleanup_prob=%.1f%%",
                   expire_minutes, cleanup_probability * 100)
    ...
    async def _cleanup_expired_if_needed(self, key: str, client) -> int:
        ...
        if random.random() < self.cleanup_probability:
            ...
            cleanup_threshold = current_timestamp - (self.default_expire_minutes * DEFAULT_CLEANUP_MULTIPLIER * 60 * 1000)
    ...
    async def append(self, 
                    ...
                    expire_minutes: Optional[int] = None) -> bool:
        """
        向指定 key 的时间窗口队列追加数据
        ...
        try:
            ...
            if zadd_result is not None and expire_result:
                if cleaned_count > 0:
                    logger.debug("数据追加成功并清理了过期数据: key=%s, member_length=%d, timestamp=%d, expire=%ds, cleaned=%d",
                               key, len(unique_member), current_timestamp, expire_seconds, cleaned_count)
                else:
                    logger.debug("数据追加成功: key=%s, member_length=%d, timestamp=%d, expire=%ds",
                               key, len(unique_member), current_timestamp, expire_seconds)
                return True
            else:
                logger.warning("数据追加部分失败: key=%s, zadd_result=%s, expire_result=%s",
                             key, zadd_result, expire_result)
                return False
    ...
    async def get_by_timestamp_range(self, 
                                   key: str, 
        ...
        try:
            ...
            messages = await self._fetch_by_timestamp_range_script(
                keys=[key],
                args=[min_score, max_score, limit]
            )
            
            if not messages:
                logger.debug("时间戳范围内无数据: key=%s, range=[%s, %s]", key, min_score, max_score)
                return []
            ...
            if len(messages) % 2 != 0:
                logger.warning("WITHSCORES返回数据长度异常: %d，应该是偶数", len(messages))
                return []
            
            for i in range(0, len(messages), 2):
                try:
                    if i + 1 >= len(messages):
                        logger.warning("WITHSCORES数据索引越界: i=%d, len=%d", i, len(messages))
                        break
                    ...
                    try:
                        ...
                        logger.warning("分数转换失败: score_raw=%s (type=%s), error=%s", 
                                     score_raw, type(score_raw), str(score_e))
                    ...
                    logger.warning("解析消息失败: i=%d, message=%s, error=%s", i, 
                                 message if i < len(messages) else "索引越界", str(e))
                    continue
            
            # 按时间戳排序（最新的在前）
            result.sort(key=lambda x: x["timestamp"], reverse=True)
            
            logger.debug("按时间戳范围获取数据成功: key=%s, range=[%s, %s], count=%d",
                        key, min_score, max_score, len(result))
    ...
    async def get_queue_stats(self, key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "oldest_datetime": datetime.fromtimestamp(oldest_timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S") if oldest_timestamp else None,
                "newest_datetime": datetime.fromtimestamp(newest_timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S") if newest_timestamp else None,
                "ttl_seconds": ttl
            }
...
class DefaultRedisWindowsCacheManager:
    ...
    async def append(self, key: str, data: Union[str, Dict, List, Any], expire_minutes: Optional[int] = None) -> bool:
