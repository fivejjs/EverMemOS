...
class RedisLengthCacheFactory:
    ...
    async def create_cache_manager(self,
                                 ...
                                 cleanup_probability: float = DEFAULT_CLEANUP_PROBABILITY) -> 'RedisLengthCacheManager':
        """
        创建缓存管理器实例
...
class RedisLengthCacheManager:
    ...
    def __init__(self, 
        ...
        logger.info("Redis 长度限制缓存管理器初始化完成: max_length=%d, expire=%d分钟, cleanup_prob=%.1f%%",
                   max_length, expire_minutes, cleanup_probability * 100)
    ...
    async def append(self, 
                    ...
                    timestamp: Optional[Union[int, datetime]] = None) -> bool:
        """
        向指定 key 的队列追加数据
        ...
        try:
            ...
            if zadd_result is not None and expire_result:
                if cleaned_count > 0:
                    logger.debug("数据追加成功并清理了最早数据: key=%s, member_length=%d, timestamp=%d, expire=%ds, cleaned=%d",
                               key, len(unique_member), score_timestamp, expire_seconds, cleaned_count)
                else:
                    logger.debug("数据追加成功: key=%s, member_length=%d, timestamp=%d, expire=%ds",
                               key, len(unique_member), score_timestamp, expire_seconds)
                return True
            else:
...
class DefaultRedisLengthCacheManager:
    ...
    async def append(self, key: str, data: Union[str, Dict, List, Any], timestamp: Optional[Union[int, datetime]] = None) -> bool:
