...
class TaskFunction:
    ...
    def to_arq_function(self) -> Function:
        """转换为arq函数"""
        return arq_func(self.coroutine, name=self.name, max_tries=self.retry_config.max_retries, timeout=self.timeout)
...
class TaskManager:
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                try:
                    ...
                    logger.debug(f"获取模块属性失败: {module.__name__}.{attr_name}, 错误: {e}")
    ...
    async def enqueue_task(
        ...
        **kwargs
        ...
        logger.info(f"已添加任务到队列: {task_id}, 任务名称: {task_name}, 用户: {user_id_for_log}")
        return task_id
