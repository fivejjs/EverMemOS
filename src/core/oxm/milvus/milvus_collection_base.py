...
class MilvusCollectionBase:
    ...
    def collection(cls) -> Collection:
        ...
        if cls._collection_instance is None:
            raise ValueError(f"{cls.__name__} 的 Collection 实例未创建，请先调用 ensure_loaded()")
    ...
    def load_collection(self, name: str) -> Collection:
        ...
        coll = Collection(name=name, using=self.using, schema=self._SCHEMA, consistency_level=ConsistencyLevel.Bounded)
    ...
    def _create_indexes_for_collection(self, coll: Collection) -> None:
        ...
        try:
            ...
            logger.info("Collection '%s' 已存在的索引字段: %s", coll.name, existing_field_names)
            ...
            for index_config in index_configs:
                ...
                logger.info("为字段 '%s' 创建索引（类型: %s）...", field_name, index_config.index_type)
                create_kwargs = {
...
class MilvusCollectionWithSuffix(MilvusCollectionBase):
    ...
    def load_collection(self, name: str) -> Collection:
        ...
        if not utility.has_collection(name, using=self._using):
            ...
            logger.info("Collection '%s' 不存在，创建新 Collection: %s", name, _collection_name)
...
if __name__ == "__main__":
    ...
    class TestCollection(MilvusCollectionWithSuffix):
        ...
        _INDEX_CONFIGS = [
            IndexConfig(field_name="embedding", index_type="IVF_FLAT", metric_type="L2", params={"nlist": 1024})
        ]
        _DB_USING = "default"
