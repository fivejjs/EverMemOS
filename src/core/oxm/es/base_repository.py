...
class BaseRepository(ABC, Generic[T]):
    ...
    async def get_client(self) -> AsyncElasticsearch:
        ...
        if self._client is None:
            try:
                ...
                logger.error("❌ 获取 Elasticsearch 客户端失败 [%s]: %s", self.model_name, e)
                raise
        return self._client
    ...
    async def create(self, document: T, refresh: bool = False) -> T:
        ...
        try:
            ...
            logger.debug("✅ 创建文档成功 [%s]: %s", self.model_name, getattr(document, 'meta', {}).get('id', 'unknown'))
            return document
        except Exception as e:
    ...
    async def delete(self, document: T, refresh: bool = False) -> bool:
        ...
        try:
            client = await self.get_client()
            await document.delete(using=client, refresh=refresh)
            logger.debug("✅ 删除文档成功 [%s]: %s", self.model_name, getattr(document, 'meta', {}).get('id', 'unknown'))
            return True
        except Exception as e:
            logger.error("❌ 删除文档失败 [%s]: %s", self.model_name, e)
            return False
    ...
    async def create_batch(self, documents: List[T], refresh: bool = False) -> List[T]:
        ...
        try:
            ...
            logger.debug("✅ 批量创建文档成功 [%s]: %d 条记录", self.model_name, len(documents))
            return documents
        except Exception as e:
    ...
    async def search(self, query: Dict[str, Any], size: int = 10, from_: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug("✅ 搜索执行成功 [%s]: 找到 %d 条结果", 
                       self.model_name, 
    ...
    async def create_index(self) -> bool:
        ...
        try:
            ...
            logger.debug("✅ 创建索引成功 [%s]: %s -> %s", self.model_name, index_name, alias)
            return True
        except Exception as e:
