...
def find_document_class_by_index_name(index_name: str) -> Type[AsyncDocument]:
    ...
    for cls in all_doc_classes:
        if hasattr(cls, "_index") and hasattr(cls._index, "_name"):
            # hardcode一下
            if cls._index._name == expected_alias and cls.__name__ != "GeneratedAliasSupportDoc":
    ...
    if len(matched_classes) > 1:
        ...
        raise ValueError(f"找到多个索引别名为 '{index_name}' 的文档类: {', '.join([f'{cls.__module__}.{cls.__name__}' for cls in matched_classes])}")
...
async def rebuild_index(document_class: Type[AsyncDocument], es_connect: Any, 
                       close_old: bool = False, delete_old: bool = False) -> None:
    """
    重建 Elasticsearch 索引
...
async def update_aliases(es_connect: Any, alias_name: str, dest_index: str, 
                        close_old: bool = False, delete_old: bool = False) -> None:
    """
