...
if BEANIE_AVAILABLE:
    class DocumentBase(Document):
        ...
        def _recursive_datetime_check(self, obj, path: str = "", depth: int = 0):
            ...
            if isinstance(obj, (list, tuple, set)):
                cls = type(obj)
                return cls(self._recursive_datetime_check(item, f"{path}[{i}]", depth + 1)
                         for i, item in enumerate(obj))

            # 情况四：对象是字典
            if isinstance(obj, dict):
                return {key: self._recursive_datetime_check(value, f"{path}[{repr(key)}]", depth + 1)
    ...
    class DocumentBase:
        ...
        """
        def __init__(self):
            raise ImportError("Beanie ODM is not available. Please install beanie to use DocumentBase.")
