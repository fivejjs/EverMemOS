...
from di import (
    component, service, repository, factory, mock_impl,
    get_bean, get_bean_by_type, get_beans_by_type,
    enable_mock_mode, disable_mock_mode,
    scan_packages, register_bean, register_factory
)
...
def demo_basic_usage():
    ...
    assert isinstance(user_service, UserServiceImpl), "获取的应该是 UserServiceImpl 实例"
    ...
    assert isinstance(notification_service, PrimaryNotificationService), "应该获取到Primary实现"
...
def demo_multiple_implementations():
    ...
    assert len(notification_services) >= 3, f"应该至少有3个通知服务实现，实际获取到{len(notification_services)}个"
    ...
    for i, service in enumerate(notification_services):
        assert isinstance(service, NotificationService), f"第{i+1}个服务应该是NotificationService的实例"
        service.send_notification(f"消息{i+1}", f"user{i+1}@example.com")
    
    # 获取Primary实现
    primary_service = get_bean_by_type(NotificationService)
    assert isinstance(primary_service, PrimaryNotificationService), "Primary实现应该是PrimaryNotificationService"
...
def demo_mock_mode():
    ...
    # 类型应该和原始的一致
    assert type(restored_user_service) == type(original_user_service), "禁用Mock模式后应该恢复原始用户服务实现"
    assert type(restored_notification_service) == type(original_notification_service), "禁用Mock模式后应该恢复原始通知服务实现"
...
def demo_factory():
    ...
    assert isinstance(factory_repo, UserRepository), "Factory创建的应该是UserRepository实例"
    assert isinstance(factory_repo, MySQLUserRepository), "Factory创建的应该是MySQLUserRepository实例"
...
def demo_advanced_features():
    ...
    assert isinstance(advanced_service, AdvancedUserService), "获取的应该是AdvancedUserService实例"
    
    # 验证依赖注入
    assert advanced_service.primary_repo is not None, "主存储依赖应该被正确注入"
    assert isinstance(advanced_service.primary_repo, MySQLUserRepository), "作为接口没有primary的匹配，所以注入的是MySQLUserRepository，工厂的优先级高于其他"
    assert advanced_service.config is not None, "配置管理器依赖应该被正确注入"
    assert isinstance(advanced_service.config, ConfigManager), "应该注入ConfigManager实例"
    assert advanced_service.notification is not None, "通知服务依赖应该被正确注入"
    assert isinstance(advanced_service.notification, PrimaryNotificationService), "应该注入Primary通知服务实现"
