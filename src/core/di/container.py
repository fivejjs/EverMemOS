...
import inspect
import abc
from typing import Dict, Type, TypeVar, Optional, Any, List, Set, Callable, Union, get_origin, get_args
from threading import RLock
from enum import Enum

from core.di.exceptions import (
    CircularDependencyError, BeanNotFoundError, DuplicateBeanError,
    FactoryError, DependencyResolutionError, MockNotEnabledError,
        ...
        with self._lock:
            ...
            if is_primary:
                ...
                if existing_primary and not existing_primary.is_mock:
                    raise PrimaryBeanConflictError(bean_type, existing_primary.bean_type, bean_type)
    ...
    def _get_candidates_with_priority(self, bean_type: Type) -> List[BeanDefinition]:
        ...
        if self._mock_mode:
            ...
            if bean_type in self._bean_definitions:
                direct_matches = [
                    bean_def for bean_def in self._bean_definitions[bean_type]
                ...
                # 每组内部按Factory优先级排序
                mock_factory_beans = [bd for bd in mock_matches if bd.scope == BeanScope.FACTORY]
                mock_regular_beans = [bd for bd in mock_matches if bd.scope != BeanScope.FACTORY]
                non_mock_factory_beans = [bd for bd in non_mock_matches if bd.scope == BeanScope.FACTORY]
                non_mock_regular_beans = [bd for bd in non_mock_matches if bd.scope != BeanScope.FACTORY]
            ...
            if bean_type in self._bean_definitions:
                ...
                # Factory优先于普通Bean
                factory_beans = [bd for bd in direct_matches if bd.scope == BeanScope.FACTORY]
                regular_beans = [bd for bd in direct_matches if bd.scope != BeanScope.FACTORY]
        ...
        for registered_type in registered_types:
            try:
                ...
                for base in registered_type.__mro__[1:]:  # 跳过自身
                    ...
                    if (base != abc.ABC and base != object and
                        hasattr(base, '__abstractmethods__')):  # ABC类型
