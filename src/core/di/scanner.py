...
class ComponentScanner:
    ...
    def __init__(self):
        ...
        self.exclude_paths: Set[str] = {'/di/', '/config/', '__pycache__', '.git', '.pytest_cache'}
    ...
    def _preload_critical_modules(self):
        ...
        self.logger.info("📦 预加载完成: %d/%d 个模块成功加载", loaded_count, len(self.preload_modules))
        if failed_count > 0:
    ...
    def scan(self) -> 'ComponentScanner':
        ...
        if self.parallel and len(python_files) > 1:
            self.logger.info("⚡ 使用并行扫描模式 (最大 %d 个工作线程)", self.max_workers)
            # 并行扫描前预加载关键模块
    ...
    def _collect_python_files(self) -> List[Path]:
        ...
        if len(python_files) != len(unique_files):
            self.logger.debug("去重后文件数量: %d -> %d", len(python_files), len(unique_files))
...
def auto_scan():
    ...
    if src_path.exists():
        ...
        for item in src_path.iterdir():
            if item.is_dir() and item.name in {'di', 'config', 'test', 'tests', '__pycache__'}:
