    ...
    async def shutdown(self, timeout: float = 30.0, wait_for_current_task: bool = True) -> None:
        ...
        if self._task and not self._task.done():
            if wait_for_current_task:
                try:
                    # 等待当前消息处理完成
                    self.logger.info("Waiting for current task to complete in consumer %s", self.job_id)
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        ...
                        self.job_id, str(handler_error), exc_info=True
                    )
                    # 如果错误处理器本身出错，短暂休眠后继续
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    ...
                    str(handler_error), exc_info=True
                )
        ...
        for attempt in range(retry_config.max_retries + 1):
            try:
                ...
                if not self._error_handler.is_retryable_error(e) and not retry_config.retry_on_fatal:
                    self.logger.error(
                        "Non-retryable error in consumer %s: %s",
                        self.job_id, str(e)
                    )
                    raise
                ...
                if attempt < retry_config.max_retries:
                    ...
                    self.logger.warning(
                        ...
                        attempt + 1, retry_config.max_retries + 1, self.job_id, delay, str(e)
                    )
                    ...
                    self.logger.error(
                        ...
                        retry_config.max_retries + 1, self.job_id, str(e)
                    )
                    raise last_error
