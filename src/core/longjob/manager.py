...
class LongJobManager:
    ...
    def __init__(self):
        ...
        self.logger.info("LongJobManager initialized with max_concurrent_jobs=%d", self.max_concurrent_jobs)
    ...
    async def add_job(self, job: LongJobInterface) -> bool:
        ...
        if len(self._jobs) >= self.max_concurrent_jobs:
            self.logger.warning(
                ...
                job.job_id, self.max_concurrent_jobs
            )
            return False
        ...
        try:
            ...
            for job in job_implementations:
                ...
                try:
                    ...
                    self.logger.error("Error adding auto-discovered job %s: %s", 
                                    getattr(job, 'job_id', 'unknown'), str(e))
        ...
        try:
            ...
            if self.log_startup_details:
                self.logger.info(
                    ...
                    results['total_discovered'], results['total_filtered'], results['total_started']
                )
            else:
                self.logger.info(
    ...
    async def shutdown(self, timeout: float = 60.0) -> None:
        ...
        try:
            # 停止所有任务
            await asyncio.wait_for(
                self.stop_all_jobs(timeout=timeout/2),
                timeout=timeout
            )
    ...
    def get_manager_stats(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            'uptime': (datetime.now() - stats['manager_start_time']).total_seconds()
        })
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    ...
                    job_id, str(cleanup_error), exc_info=True
                )
