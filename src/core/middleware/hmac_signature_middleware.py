...
class HMACSignatureMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, secret_key: str, time_window_minutes: int = 5, redis_provider: Optional[RedisProvider] = None):
        """
        初始化HMAC签名中间件
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.error(
                ...
                "method='%s', url_path='%s'",
                str(e), timestamp_header, nonce_header, signature_header,
        ...
        try:
            ...
            if token is not None:
                try:
                    ...
                    logger.warning(f"清理HMAC签名用户上下文时发生错误: {str(reset_error)}")
    
    async def _verify_hmac_signature(self, request: Request, timestamp_header: str, nonce_header: str, signature_header: str) -> bool:
        """
        验证HMAC签名
        ...
        if self.redis_provider:
            try:
                ...
                nonce_stored = await self.redis_provider.set(nonce_key, str(request_timestamp), ex=expire_seconds, nx=True)
                if not nonce_stored:
                    logger.error(
                        ...
                        timestamp_header, nonce_header, signature_header,
                        request.method, request.url.path, request_timestamp, int(time.time())
                    )
                    return False
                logger.debug("nonce已存储到Redis: %s, 过期时间=%d秒", nonce_header, expire_seconds)
            except Exception as e:
        ...
        if time_diff > self.time_window_seconds:
            logger.error(
                ...
                timestamp_header, nonce_header, signature_header,
                request.method, request.url.path, current_time, request_timestamp,
        ...
        if not hmac.compare_digest(signature_header, expected_signature):
            logger.error(
                ...
                timestamp_header, nonce_header, signature_header,
                request.method, request.url.path, signature_data,
                expected_signature, len(self.secret_key)
            )
            return False
...
def create_hmac_middleware(secret_key: str, time_window_minutes: int = 5, redis_provider: Optional[RedisProvider] = None):
    """
    创建HMAC签名中间件的工厂函数
