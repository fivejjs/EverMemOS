...
class SmartTextParser:
    ...
    def _is_punctuation(self, char: str) -> bool:
        ...
        return char in punctuation_chars or (
            0x2000 <= ord(char) <= 0x206F or  # 通用标点
            0x3000 <= ord(char) <= 0x303F or  # CJK符号和标点
            0xFF00 <= ord(char) <= 0xFFEF     # 全角ASCII和半角片假名
        )
        ...
        while i < text_len:
            ...
            # 处理英文单词
                ...
                while (word_end < text_len and 
                       (self._is_english_char(text[word_end]) or 
                        text[word_end] in "'-")):
                    word_end += 1
    ...
    def smart_truncate_by_score(self, text: str, max_score: float, 
                               suffix: str = "...", enable_fallback: bool = True) -> str:
        """基于分数智能截断文本
        ...
        try:
            ...
            for token in tokens:
                if current_score + token.score > max_score:
                    ...
                    if (token.type in [TokenType.ENGLISH_WORD, TokenType.CONTINUOUS_NUMBER] and 
                        current_score + token.score <= max_score * 1.05 and  # 只允许5%超出
                        current_score > 0):  # 必须已经有其他token，不能第一个token就超出
                        current_score += token.score
            ...
            if enable_fallback:
                ...
                # 向后查找合适的截断点（空白或标点）
                for i in range(min(estimated_length + 10, len(text) - 1), 
                              max(estimated_length - 10, 0), -1):
...
def smart_truncate_text(text: str, max_count: int, chinese_weight: float = 1.0, english_word_weight: float = 1.0, suffix: str = "...") -> str:
    """
    基于单词/字符计数的智能截取文本
