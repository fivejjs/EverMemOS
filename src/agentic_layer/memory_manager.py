...
from .dtos.memory_query import FetchMemRequest, FetchMemResponse, RetrieveMemRequest, RetrieveMemResponse, Metadata
...
class MemoryManager:
    ...
    async def retrieve_mem(self, retrieve_mem_request: 'RetrieveMemRequest') -> RetrieveMemResponse:
        ...
        try:
            ...
            logger.info(f"retrieve_mem 分发请求: user_id={retrieve_mem_request.user_id}, "
    ...
    async def retrieve_mem_keyword(self, retrieve_mem_request: 'RetrieveMemRequest') -> RetrieveMemResponse:
        ...
        try:
            # 从 Request 中获取参数
            if not retrieve_mem_request:
                raise ValueError("retrieve_mem_request is required for retrieve_mem_keyword")
            
            search_results = await self.get_keyword_search_results(retrieve_mem_request)
            
            if not search_results:
                logger.warning(f"关键词检索未找到结果: user_id={retrieve_mem_request.user_id}, query={retrieve_mem_request.query}")
                return RetrieveMemResponse(
                    memories=[],
                    original_data=[],
                    scores=[],
            ...
            memories, scores, importance_scores, original_data, total_count = await self.group_by_groupid_stratagy(
                search_results, source_type="es"
            )
            
            logger.debug(f"EpisodicMemoryEsRepository multi_search returned {len(memories)} groups for query: {retrieve_mem_request.query}")
    ...
    async def get_keyword_search_results(self, retrieve_mem_request: 'RetrieveMemRequest') -> Dict[str, Any]:
        try:
            # 从 Request 中获取参数
            if not retrieve_mem_request:
                raise ValueError("retrieve_mem_request is required for retrieve_mem")
    ...
    async def retrieve_mem_vector(self, retrieve_mem_request: 'RetrieveMemRequest') -> RetrieveMemResponse:
        ...
        try:
            # 从 Request 中获取参数
            logger.debug(f"retrieve_mem_vector called with retrieve_mem_request: {retrieve_mem_request}")
            if not retrieve_mem_request:
                raise ValueError("retrieve_mem_request is required for retrieve_mem_vector")
            ...
            logger.debug(f"retrieve_mem_vector called with query: {query}, user_id: {user_id}, top_k: {top_k}")
            ...
            logger.debug(f"EpisodicMemoryMilvusRepository vector_search returned {len(memories)} groups for query: {query}")
            ...
            return RetrieveMemResponse(
                ...
                total_count=0,
                has_more=False,
                query_metadata=Metadata(source="retrieve_mem_vector_service", user_id=user_id if 'user_id' in locals() else "", memory_type="retrieve_vector"),
                metadata=Metadata(source="retrieve_mem_vector_service", user_id=user_id if 'user_id' in locals() else "", memory_type="retrieve_vector")
            )

    async def get_vector_search_results(self, retrieve_mem_request: 'RetrieveMemRequest') -> Dict[str, Any]:
        try:
            # 从 Request 中获取参数
    ...
    async def retrieve_mem_hybrid(self, retrieve_mem_request: 'RetrieveMemRequest') -> RetrieveMemResponse:
        ...
        """
        try:
            logger.debug(f"retrieve_mem_hybrid called with retrieve_mem_request: {retrieve_mem_request}")
            if not retrieve_mem_request:
                raise ValueError("retrieve_mem_request is required for retrieve_mem_hybrid")
            ...
            logger.debug(f"retrieve_mem_hybrid called with query: {query}, user_id: {user_id}, top_k: {top_k}")
            ...
            # 并行执行两种检索，获取原始搜索结果
            keyword_search_results = await self.get_keyword_search_results(keyword_request)
            ...
            return RetrieveMemResponse(
                ...
                total_count=0,
                has_more=False,
                query_metadata=Metadata(source="retrieve_mem_hybrid_service", user_id=user_id if 'user_id' in locals() else "", memory_type="retrieve_hybrid"),
                metadata=Metadata(source="retrieve_mem_hybrid_service", user_id=user_id if 'user_id' in locals() else "", memory_type="retrieve_hybrid")
            )
    ...
    async def _merge_and_rerank_search_results(
        ...
        try:
            ...
            reranked_hits = await rerank_service._rerank_all_hits(query, all_hits, top_k)
            ...
            reranked_hits = sorted(all_hits, key=self._extract_score_from_hit, reverse=True)[:top_k]
        
        # 对rerank后的结果进行分组处理
        memories, scores, importance_scores, original_data, total_count = await self.group_by_groupid_stratagy(
            reranked_hits, source_type="hybrid"
        )
    ...
    async def group_by_groupid_stratagy(self, search_results: List[Dict[str, Any]], source_type: str = "milvus") -> tuple:
        """通用的搜索结果分组处理策略
        ...
        for hit in search_results:
            ...
            if timestamp_raw:
                if isinstance(timestamp_raw, datetime):
                    ...
                    try:
                        ...
                        logger.warning(f"timestamp为数字但转换失败: {timestamp_raw}, error: {e}")
                        timestamp = datetime.now().replace(tzinfo=None)
                elif isinstance(timestamp_raw, str):
                    try:
                        timestamp = from_iso_format(timestamp_raw).replace(tzinfo=None)
                    except Exception as e:
                        logger.warning(f"timestamp格式转换失败: {timestamp_raw}, error: {e}")
                        timestamp = datetime.now().replace(tzinfo=None)
                else:
                    logger.warning(f"未知类型的timestamp_raw: {type(timestamp_raw)}, 使用当前时间")
                    timestamp = datetime.now().replace(tzinfo=None)
            else:
                timestamp = datetime.now().replace(tzinfo=None)
            ...
            if user_id and group_id:
                try:
                    ...
                    group_user_profile = await group_user_profile_repo.get_by_user_group(user_id, group_id)
                    
                    if group_user_profile and hasattr(group_user_profile, 'group_importance_evidence') and group_user_profile.group_importance_evidence:
                        ...
                        logger.debug(f"为memory添加group_importance_evidence: user_id={user_id}, group_id={group_id}")
                    else:
                        logger.debug(f"未找到group_importance_evidence: user_id={user_id}, group_id={group_id}")
                except Exception as e:
                    logger.warning(f"读取group_user_profile_memory失败: user_id={user_id}, group_id={group_id}, error={e}")
        ...
        for group_id, group_data in memories_by_group.items():
            ...
            # 计算重要性得分
            importance_score = calculate_importance_score(group_data['importance_evidence'])
