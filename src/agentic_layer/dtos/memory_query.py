...
from agentic_layer.memory_models import MemoryType, Metadata, MemoryModel, RetrieveMethod
...
class FetchMemRequest:
    ...
    version_range: Optional[tuple[Optional[str], Optional[str]]] = None  # 版本范围 (start, end)，左闭右闭区间 [start, end]
...
class RetrieveMemResponse:
    ...
    original_data: List[Dict[str, List[Dict[str, Any]]]] = field(default_factory=list)  # 新增：原始数据
    total_count: int = 0
    has_more: bool = False
