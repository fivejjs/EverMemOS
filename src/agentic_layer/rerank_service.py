...
class DeepInfraRerankConfig:
    ...
    def __post_init__(self):
        ...
        if not self.base_url:
            self.base_url = os.getenv("DEEPINFRA_RERANK_BASE_URL", "https://api.deepinfra.com/v1/inference")
        if not self.model:
        ...
        if self.max_concurrent_requests == 5:  # 使用默认值时才从环境变量读取
            self.max_concurrent_requests = int(os.getenv("DEEPINFRA_RERANK_MAX_CONCURRENT", "5"))
...
class RerankMemResponse:
    ...
    rerank_scores: List[Dict[str, List[float]]] = field(default_factory=list)  # 重排序分数
...
class DeepInfraRerankServiceInterface(ABC):
    ...
    @abstractmethod
    async def rerank_memories(self, query: str, retrieve_response: Any) -> RerankMemResponse:
        """
        对检索到的记忆进行重排序
...
class DeepInfraRerankService(DeepInfraRerankServiceInterface):
    ...
    def _load_config_from_env(self) -> DeepInfraRerankConfig:
        ...
        return DeepInfraRerankConfig(
            ...
            base_url=os.getenv("DEEPINFRA_RERANK_BASE_URL", "https://api.deepinfra.com/v1/inference"),
            ...
            max_concurrent_requests=int(os.getenv("DEEPINFRA_RERANK_MAX_CONCURRENT", "5"))
        )
                ...
                try:
                    async with self.session.post(url, json=request_data) as response:
                        if response.status == 200:
                            ...
                            logger.error(f"DeepInfra Rerank API error (attempt {attempt + 1}): {response.status} - {error_text}")
                            
                            if attempt < self.config.max_retries - 1:
                                await asyncio.sleep(2 ** attempt)  # 指数退避
                                continue
                            else:
                                raise DeepInfraRerankError(f"Rerank API request failed: {response.status} - {error_text}")
                
                except aiohttp.ClientError as e:
                    logger.error(f"DeepInfra Rerank API client error (attempt {attempt + 1}): {e}")
                    if attempt < self.config.max_retries - 1:
                    ...
                    logger.error(f"Unexpected rerank error (attempt {attempt + 1}): {e}")
                    if attempt < self.config.max_retries - 1:
                        await asyncio.sleep(2 ** attempt)
                        continue
                    else:
    ...
    def _convert_response_format(self, api_response: Dict[str, Any], num_documents: int) -> Dict[str, Any]:
        """
        ...
        if len(scores) != num_documents:
            logger.warning(f"API返回的scores数量({len(scores)})与文档数量({num_documents})不匹配")
            # 如果scores不足，用0填充
    ...
    async def rerank_memories(self, query: str, retrieve_response: Any, instruction: str = None) -> Union[RerankMemResponse, List[Dict[str, Any]]]:
        """
        对检索到的记忆进行重排序
        ...
        for group_idx, memory_dict_by_group in enumerate(retrieve_response.memories):
            for group_id, memory_list in memory_dict_by_group.items():
                for mem_idx, memory in enumerate(memory_list):
                    ...
                    memory_to_group_index[len(all_memories) - 1] = (group_idx, group_id, mem_idx)
        ...
        try:
            ...
            if "results" not in rerank_result:
                raise DeepInfraRerankError("Invalid rerank API response: missing results field")
            
            # 解析重排序结果
            rerank_scores = [item.get("relevance_score", 0.0) for item in rerank_result["results"]]
            rerank_indices = [item.get("index", i) for i, item in enumerate(rerank_result["results"])]
            ...
            for new_idx, original_idx in enumerate(rerank_indices):
                ...
                # 获取原始数据
                original_data_list = retrieve_response.original_data[group_idx].get(group_id, [])
            ...
            reranked_importance_scores = getattr(retrieve_response, 'importance_scores', [])
            ...
            return RerankMemResponse(
                ...
                total_count=getattr(retrieve_response, 'total_count', len(all_memories)),
                has_more=getattr(retrieve_response, 'has_more', False),
                query_metadata=getattr(retrieve_response, 'query_metadata', {}),
                metadata=getattr(retrieve_response, 'metadata', {})
            )
    ...
    async def _rerank_all_hits(self, query: str, all_hits: List[Dict[str, Any]], top_k: int = None, instruction: str = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            sorted_hits = sorted(all_hits, key=self._extract_score_from_hit, reverse=True)
            if top_k is not None and top_k > 0:
