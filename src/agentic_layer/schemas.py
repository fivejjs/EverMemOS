...
class RequestType(str, Enum):
    ...
    RETRIEVE_DYNAMIC_MEM_KEYWORD = "retrieve_dynamic_mem_keyword" # 使用 BM25 进行动态记忆检索
    RETRIEVE_DYNAMIC_MEM_VECTOR = "retrieve_dynamic_mem_vector" # 使用向量进行动态记忆检索
    RETRIEVE_STATIC_MEM_KEYWORD = "retrieve_static_mem_keyword" # 使用 BM25 进行静态记忆检索
    RETRIEVE_STATIC_MEM_VECTOR = "retrieve_static_mem_vector" # 使用向量进行静态记忆检索
    RETRIEVE_DYNAMIC_MEM_MIX = "retrieve_dynamic_mem_mix" # 使用混合检索策略进行动态记忆检索
    RETRIEVE_STATIC_MEM_MIX = "retrieve_static_mem_mix" # 使用混合检索策略进行静态记忆检索
...
class Request:
    ...
    request_entrypoint_type: Optional[RequestEntrypointType] = RequestEntrypointType.REST
    ...
    source: AppType = AppType.UNKNOWN # retrieve和fetch会用到，可能不同来源对应不同的场景
    ...
    def __post_init__(self) -> None:
        ...
        if self.request_type == RequestType.MEMORIZE:
            if self.memorize_request is None:
                raise ValueError("Request.memorize_request is required when request_type=MEMORIZE")
        elif self.request_type == RequestType.FETCH_MEM:
            if self.fetch_mem_request is None:
                raise ValueError("Request.fetch_mem_request is required when request_type=FETCH_MEM")
        elif self.request_type == RequestType.FETCH_AND_RETRIEVE:
            if self.fetch_mem_request is None:
                raise ValueError("Request.fetch_mem_request is required when request_type=FETCH_AND_RETRIEVE")
            if self.retrieve_mem_request is None:
                raise ValueError("Request.retrieve_mem_request is required when request_type=FETCH_AND_RETRIEVE")
