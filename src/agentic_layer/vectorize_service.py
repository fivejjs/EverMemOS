...
class DeepInfraConfig:
    ...
    def __post_init__(self):
        ...
        if not self.model:
            self.model = os.getenv("DEEPINFRA_EMBEDDING_MODEL", "Qwen/Qwen3-Embedding-4B")
        ...
        if self.max_concurrent_requests == 5:  # 使用默认值时才从环境变量读取
            self.max_concurrent_requests = int(os.getenv("DEEPINFRA_MAX_CONCURRENT", "5"))
...
class DeepInfraVectorizeServiceInterface(ABC):
    ...
    @abstractmethod
    async def get_embedding_with_usage(self, text: str, instruction: Optional[str] = None) -> Tuple[np.ndarray, Optional[UsageInfo]]:
        """
    ...
    @abstractmethod
    async def get_embeddings_batch(self, text_batches: List[List[str]], instruction: Optional[str] = None) -> List[List[np.ndarray]]:
        """
...
class DeepInfraVectorizeService(DeepInfraVectorizeServiceInterface):
    ...
    def __init__(self, config: Optional[DeepInfraConfig] = None):
        ...
        # 确保模型非空
        model = config.model or os.getenv("DEEPINFRA_EMBEDDING_MODEL", "Qwen/Qwen3-Embedding-4B")
    ...
    def _parse_embeddings_response(self, response) -> List[np.ndarray]:
        ...
        for i, item in enumerate(response.data):
            if not hasattr(item, 'embedding'):
                raise DeepInfraError(f"Invalid API response: data[{i}] missing 'embedding' field")
            
            embedding_data = item.embedding
            if not isinstance(embedding_data, list):
                raise DeepInfraError(f"Invalid API response: data[{i}].embedding should be a list")
                
            if not embedding_data:
                raise DeepInfraError(f"Invalid API response: data[{i}].embedding is empty")
            
            # 验证embedding数据都是数字
            try:
                embedding_array = np.array(embedding_data, dtype=np.float32)
            except (ValueError, TypeError) as e:
                raise DeepInfraError(f"Invalid API response: data[{i}].embedding contains non-numeric values: {e}")
...
# 便捷函数
async def get_text_embedding(text: str, instruction: Optional[str] = None) -> np.ndarray:
    """
...
async def get_text_embeddings(texts: List[str], instruction: Optional[str] = None) -> List[np.ndarray]:
    """
...
async def get_text_embeddings_batch(text_batches: List[List[str]], instruction: Optional[str] = None) -> List[List[np.ndarray]]:
    """
...
async def get_text_embedding_with_usage(text: str, instruction: Optional[str] = None) -> Tuple[np.ndarray, Optional[UsageInfo]]:
    """
