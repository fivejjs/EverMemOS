...
def migrate_data_callback(
    ...
    """
    logger.info("开始迁移数据: %s -> %s (批大小: %d)", 
                old_collection.name, new_collection.name, batch_size)
    ...
    while True:
        ...
        logger.info("查询第 %d 批数据，起始ID: %s", 
                    total_migrated // batch_size + 1, last_id if last_id else "开始")
        ...
        if query_result:
            # 找到当前批次的最大ID（按字符串排序）
            current_ids = [record.get("id", record.get("pk", "")) for record in query_result]
...
示例用法:
  ...
  python milvus_rebuild_collection.py -a episodic_memory --drop-old
    ...
    parser.add_argument(
        "--alias", "-a",
        required=True,
        help="Collection 别名，如: episodic_memory"
    )
    parser.add_argument(
        "--drop-old", "-x",
        action="store_true",
