...
class GroupChatMemorizer:
    ...
    async def process_with_v3_api_single(
        self,
...
接口说明:
  ...
  - V2 接口（兼容）：/api/v2/agentic/memorize - 需要先转换为内部格式，逐条处理
    ...
    parser.add_argument(
        '--api-url',
        type=str,
        help='memorize API地址（必需，除非使用 --validate-only）'
    )
    parser.add_argument(
        '--use-v2',
        action='store_true',
    ...
    if not args.api_url:
        ...
        logger.error("   使用方法:")
        logger.error("     V3（推荐）: python src/bootstrap.py src/run_memorize.py --input <file> --api-url http://localhost:1995/api/v3/agentic/memorize")
        logger.error("     V2 兼容: python src/bootstrap.py src/run_memorize.py --input <file> --api-url http://localhost:1995/api/v2/agentic/memorize --use-v2")
