...
from memory_layer.memory_extractor.profile_memory_extractor import GroupImportanceEvidence, ImportanceEvidence
...
from infra_layer.adapters.out.persistence.document.memory.memcell import MemCell as DocMemCell, RawData as DocRawData, DataTypeEnum
...
from common_utils.datetime_utils import get_now_with_timezone, to_timezone, to_iso_format, from_iso_format, from_timestamp
...
def _convert_importance_evidence_to_document(importance_evidence_list: List[ImportanceEvidence]) -> List[Dict[str, Any]]:
    """
...
def _convert_document_to_importance_evidence(importance_evidence_list: List[Dict[str, Any]]) -> List[ImportanceEvidence]:
    """
    将数据库文档格式转换为ImportanceEvidence
    """
    if not importance_evidence_list:
        return None
    return [ImportanceEvidence(
        user_id=importance_evidence["user_id"],
        group_id=importance_evidence["group_id"],
        speak_count=importance_evidence["speak_count"],
        refer_count=importance_evidence["refer_count"],
...
def _convert_group_importance_evidence_to_document(group_importance_evidence: GroupImportanceEvidence) -> Dict[str, Any]:
    """
...
def _convert_document_to_group_importance_evidence(group_importance_evidence: Dict[str, Any]) -> GroupImportanceEvidence:
    """
...
def _convert_group_profile_data_to_profile_format(group_profile_memory: GroupProfileMemory) -> Dict[str, Any]:
    """
    ...
    """
    from infra_layer.adapters.out.persistence.document.memory.group_profile import TopicInfo as DocTopicInfo
    ...
    from infra_layer.adapters.out.persistence.document.memory.group_profile import RoleAssignment
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                if isinstance(parsed_value, list):
                    # 检查是否已经是规范格式
                    if parsed_value and isinstance(parsed_value[0], dict) and ("value" in parsed_value[0] or "evidences" in parsed_value[0]):
                        return parsed_value
                    # 否则转换为规范格式
...
def _convert_projects_participated_list(projects_participated: Optional[List[Dict[str, str]]]) -> List[ProjectInfo]:
    """
...
def _convert_memcell_to_document(memcell: MemCell, current_time: Optional[datetime] = None) -> DocMemCell:
    ...
    try:
        ...
        if memcell.type == RawDataType.CONVERSATION:
            for raw_data_dict in memcell.original_data:
                ...
                message = {
                    ...
                    "isReplySuggest": to_string(raw_data_dict.get('isReplySuggest', '')),
                    "readUpdateTime": to_string(raw_data_dict.get('readUpdateTime', '')),
                    }
                }
        ...
        # 动态导入以避免循环依赖
        try:
            from memory_layer.memcell_extractor.linkdoc_memcell_extractor import LinkDocMemCell
            from memory_layer.memcell_extractor.email_memcell_extractor import EmailMemCell
            ...
            if isinstance(memcell, LinkDocMemCell):
                ...
                logger.debug(f"提取LinkDocMemCell扩展字段: {list(linkdoc_fields.keys())}")
...
@trace_logger(operation_name="mem_db_operations _get_raw_data_by_time_range", log_level="info")
async def _get_raw_data_by_time_range(group_id: str, start_time: datetime, end_time: datetime, limit: int = 1000) -> List[RawData]:
    """
    根据时间范围获取RawData,前闭后开
    """
    repository = get_bean_by_type(ConversationDataRepository)
    return await repository.get_conversation_data(group_id=group_id, start_time=start_time, end_time=end_time, limit=limit)
...
async def _save_group_profile_memory(group_profile_memory: GroupProfileMemory, group_profile_raw_repo: GroupProfileRawRepository, version: Optional[str] = None) -> None:
    """
...
async def _save_profile_memory_to_core(profile_memory: ProfileMemory, core_memory_repo: CoreMemoryRawRepository, version: Optional[str] = None) -> None:
    """
    ...
    try:
        ...
        save_data = {
            "extend": getattr(profile_memory, 'extend', None)
        }
        # 添加非空字段
        for k, v in converted_data.items():
            if v is not None:
...
async def _save_profile_memory_to_group_user_profile_memory(profile_memory: ProfileMemory, group_user_profile_memory_repo: GroupUserProfileMemoryRawRepository, version: Optional[str] = None) -> None:
    """
    将ProfileMemory保存到CoreMemoryRawRepository
    ...
    try:
        ...
        # 使用upsert_by_user_id方法（如果存在则更新，不存在则创建）
        await group_user_profile_memory_repo.upsert_by_user_group(profile_memory.user_id, profile_memory.group_id, save_data)
...
async def _update_status_for_new_conversation(status_repo: ConversationStatusRawRepository, request: MemorizeRequest, earliest_time: str, current_time: datetime) -> bool:
    """
    为新对话创建状态记录
...
async def _update_status_for_continuing_conversation(status_repo: ConversationStatusRawRepository, request: MemorizeRequest, latest_time: str, current_time: datetime) -> bool:
    """
    ...
    try:
        ...
        update_data = {
            "old_msg_start_time": _normalize_datetime_for_storage(existing_status.old_msg_start_time) if existing_status.old_msg_start_time else None,
            "new_msg_start_time": new_msg_start_time + timedelta(milliseconds=1),
            "last_memcell_time": _normalize_datetime_for_storage(existing_status.last_memcell_time) if existing_status.last_memcell_time else None,
...
async def _update_status_after_memcell_extraction(status_repo: ConversationStatusRawRepository, request: MemorizeRequest, memcell_time: str, current_time: datetime) -> bool:
    """
            ...
            if hasattr(raw_data, 'messages') and raw_data.messages:
                for message in raw_data.messages:
                    if hasattr(message, 'content') and message.content:
                        ...
                        message_data = {
                            "speaker_name": extend.get('sender', extend.get('speaker_name', '')),
                            "speaker_id": extend.get('speaker_id', extend.get('user_id', '')),
...
def _convert_group_profile_raw_to_memory_format(doc_group_profile_raw) -> Dict[str, Any]:
    """
    ...
    if hasattr(doc_group_profile_raw, 'topics') and doc_group_profile_raw.topics:
        ...
        for topic_data in doc_group_profile_raw.topics:
            if isinstance(topic_data, dict):
                ...
                # 如果是对象，转换为TopicInfo
                try:
                    topic_dict = topic_data.__dict__ if hasattr(topic_data, '__dict__') else {}
    ...
    if hasattr(doc_group_profile_raw, 'roles') and doc_group_profile_raw.roles:
        ...
        if isinstance(doc_group_profile_raw.roles, dict):
            ...
            # 如果不是字典，尝试转换
            try:
                roles = dict(doc_group_profile_raw.roles) if doc_group_profile_raw.roles else {}
            except Exception as e:
    ...
    if hasattr(doc_group_profile_raw, 'extend') and doc_group_profile_raw.extend:
        if isinstance(doc_group_profile_raw.extend, dict):
            ...
            # 如果不是字典，尝试转换
            try:
                extend = dict(doc_group_profile_raw.extend) if doc_group_profile_raw.extend else {}
            except Exception as e:
    ...
    if hasattr(doc_group_profile_raw, 'timestamp') and doc_group_profile_raw.timestamp:
        if isinstance(doc_group_profile_raw.timestamp, datetime):
            ...
            logger.warning(f"无法转换timestamp: {type(doc_group_profile_raw.timestamp)}")
