...
from memory_layer.memory_extractor.group_profile_memory_extractor import GroupProfileMemoryExtractor, GroupProfileMemoryExtractRequest, GroupProfileMemory
...
from infra_layer.adapters.out.search.elasticsearch.converter.episodic_memory_converter import EpisodicMemoryConverter
from infra_layer.adapters.out.search.milvus.converter.episodic_memory_milvus_converter import EpisodicMemoryMilvusConverter
    ...
    if not request.new_raw_data_list:
        ...
        if doc_status:
            conversation_status = ConversationStatus(
                ...
                old_msg_start_time=_convert_timestamp_to_time(doc_status.old_msg_start_time),
                new_msg_start_time=_convert_timestamp_to_time(doc_status.new_msg_start_time),
                last_memcell_time=_convert_timestamp_to_time(doc_status.last_memcell_time),
                created_at=doc_status.created_at.isoformat() if hasattr(doc_status, 'created_at') and doc_status.created_at else to_iso_format(current_time),
                updated_at=doc_status.updated_at.isoformat() if hasattr(doc_status, 'updated_at') and doc_status.updated_at else to_iso_format(current_time)
            )
        ...
        if conversation_status:
            ...
            old_msg_start_time = _normalize_datetime_for_storage(conversation_status.old_msg_start_time)
            new_msg_start_time = _normalize_datetime_for_storage(conversation_status.new_msg_start_time)
            ...
            if request.new_raw_data_list and new_msg_start_time:
                ...
                for raw_data in request.new_raw_data_list:
                    ...
                    if message_time and (earliest_new_message_time is None or message_time < earliest_new_message_time):
                ...
                if earliest_new_message_time:
                    ...
                    if new_msg_start_time and earliest_new_message_time < new_msg_start_time:
                        logger.debug(f"[mem_memorize] 检测到更早的消息: {earliest_new_message_time} < {new_msg_start_time}")
                        ...
                        new_boundary = earliest_new_message_time - timedelta(milliseconds=1)
                        old_msg_start_time = min(old_msg_start_time, new_boundary)
                        
                        logger.debug(f"[mem_memorize] 时间边界已调整: old_msg_start_time={old_msg_start_time}, new_msg_start_time={new_msg_start_time}")
                        
                        # 时间边界调整后，更新conversation status表
                        try:
                            update_data = {
                                "old_msg_start_time": _normalize_datetime_for_storage(old_msg_start_time),
                                "new_msg_start_time": _normalize_datetime_for_storage(new_msg_start_time),
                                "updated_at": current_time
                            }
                            
                            result = await status_repo.upsert_by_group_id(request.group_id, update_data)
                            if result:
                                logger.debug(f"[mem_memorize] 时间边界调整后，conversation status表更新成功")
                            else:
                                logger.debug(f"[mem_memorize] 时间边界调整后，conversation status表更新失败")
                        except Exception as e:
                            logger.debug(f"时间边界调整后，更新conversation status表异常: {e}")
            ...
            if new_msg_start_time:
                new_data = await _get_raw_data_by_time_range(
                    ...
                    end_time=_normalize_datetime_for_storage(current_time) + timedelta(milliseconds=1),  # 添加结束时间为当前时间
                    limit=500  # 限制新消息数量
                )
            # 移除高频调试日志
            logger.info(f"[mem_memorize] 从状态表重新读取: 历史数据 {len(history_data)} 条, 新数据 {len(new_data)} 条")
            ...
            logger.info(f"[mem_memorize] 重新分配: 历史数据 {len(history_raw_data_list)} 条, 新数据 {len(new_raw_data_list)} 条")
            ...
            if request.new_raw_data_list:
                ...
                if hasattr(first_msg, 'content') and isinstance(first_msg.content, dict):
                    earliest_new_time = first_msg.content.get('timestamp', earliest_new_time)
                elif hasattr(first_msg, 'timestamp'):
                    earliest_new_time = first_msg.timestamp
            
            # 使用封装函数创建新对话状态
            await _update_status_for_new_conversation(status_repo, request, earliest_new_time, current_time)
        # 4. 检查是否有数据需要处理
...
async def update_status_when_no_memcell(request: MemorizeRequest, status_result: StatusResult, current_time: datetime, data_type: RawDataType):
...
async def update_status_after_memcell(request: MemorizeRequest, memcells: List[MemCell], current_time: datetime, data_type: RawDataType):
    if data_type == RawDataType.CONVERSATION:
...
async def save_memories(memory_list: List[Memory], current_time: datetime, version: Optional[str] = None):
    logger.info(f"[mem_memorize] 保存 {len(memory_list)} 个记忆到数据库")
    # 初始化Repository实例
    episodic_memory_repo = get_bean_by_type(EpisodicMemoryRawRepository)
    group_user_profile_memory_repo = get_bean_by_type(GroupUserProfileMemoryRawRepository)
    ...
    # 按memory_type分类保存
    episode_memories = [m for m in memory_list if m.memory_type == MemoryType.EPISODE_SUMMARY]
    profile_memories = [m for m in memory_list if m.memory_type == MemoryType.PROFILE]
    group_profile_memories = [m for m in memory_list if m.memory_type == MemoryType.GROUP_PROFILE]
    ...
    for episode_mem in episode_memories:
        try:
            ...
            if not vector or (isinstance(vector, list) and len(vector) == 0):
                logger.warning("[mem_memorize] 跳过写入Milvus：向量为空或缺失，event_id=%s", getattr(doc, 'event_id', None))
            else:
    ...
    for profile_mem in profile_memories:
        try:
            await _save_profile_memory_to_group_user_profile_memory(profile_mem, group_user_profile_memory_repo, version)
        except Exception as e:
            logger.error(f"保存Profile记忆失败: {e}")

    for group_profile_mem in group_profile_memories:
        try:
            await _save_group_profile_memory(group_profile_mem, group_profile_raw_repo, version)
        except Exception as e:
        ...
        for user_id, core_memory in user_core_memories.items():
            if core_memory:
                ...
                profile_memory = ProfileMemory(
                    ...
                    projects_participated=_convert_projects_participated_list(getattr(core_memory, 'projects_participated', None)),
                )
                old_memory_list.append(profile_memory)
        
        logger.info(f"[mem_memorize] 直接转换了 {len(old_memory_list)} 个CoreMemory为ProfileMemory")
    else:
...
async def memorize(request: MemorizeRequest) -> List[Memory]:
    ...
    if request.raw_data_type == RawDataType.CONVERSATION:
        ...
        now = time.time()
        logger.debug(f"[memorize memorize] 提取MemCell开始: group_id={request.group_id}, group_name={request.group_name}")
        memcell_result = await memory_manager.extract_memcell(request.history_raw_data_list, request.new_raw_data_list, request.raw_data_type, request.group_id, request.group_name, request.user_id_list)
    ...
    if(if_memorize(memcells)):
        ...
        await update_status_after_memcell(request, memcells, current_time, request.raw_data_type)
...
async def memorize_offline_by_group_id(group_id: str, memcells: List[MemCell], old_memory_list: List[Memory]) -> List[Memory]:
    """
    按群组ID进行离线记忆处理
    ...
    Returns:
        提取的记忆列表
    """
    logger.info(f"[memorize_offline_by_group_id] 开始处理群组: {group_id}, MemCell数量: {len(memcells)}, 历史记忆数量: {len(old_memory_list)}")
    ...
    logger.info(f"[memorize_offline_by_group_id] 群组 {group_id} 历史记忆类型统计: {memory_type_counts}")
    ...
    logger.info(f"[memorize_offline_by_group_id] 群组 {group_id} 参与者: {participants}")

    user_organization = await _get_user_organization(participants)
    group_repo = get_bean_by_type(ChatGroupRawRepository)
    now = time.time()
    logger.debug(f"[memorize_offline_by_group_id] 获取群组名称开始: group_id={group_id}")
    group_name = await group_repo.get_name_by_id(group_id)
    logger.debug(f"[memorize_offline_by_group_id] 获取群组名称耗时: {time.time() - now}秒")
    # 初始化记忆提取器
    memory_manager = MemoryManager()
    ...
    try:
        ...
        logger.info(f"[memorize_offline_by_group_id] 已创建群组档案记忆提取任务: {group_id}")
    ...
    try:
        ...
        logger.info(f"[memorize_offline_by_group_id] 已创建档案记忆提取任务: {group_id}")
    ...
    # 并行执行所有任务
    if not tasks:
        logger.warning(f"[memorize_offline_by_group_id] 群组 {group_id} 没有可执行的任务")
        return []
    
    logger.info(f"[memorize_offline_by_group_id] 开始并行执行 {len(tasks)} 个任务: {group_id}")
    ...
    try:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                logger.error(f"[memorize_offline_by_group_id] 任务 {task_name} 执行失败: {result}")
            elif result is None:
                logger.warning(f"[memorize_offline_by_group_id] 任务 {task_name} 返回空结果")
            elif isinstance(result, list):
                extracted_memories.extend(result)
                # 移除单个任务成功的日志
            else:
                logger.warning(f"[memorize_offline_by_group_id] 任务 {task_name} 返回未知格式结果: {type(result)}")
        
        logger.info(f"[memorize_offline_by_group_id] 群组 {group_id} 处理完成，共提取 {len(extracted_memories)} 个记忆")
...
async def memorize_offline(request: MemorizeOfflineRequest) -> List[Memory]:
    ...
    logger.info(f"[memorize_offline] 开始读取MemCell数据: 时间范围 {request.memorize_from} ~ {request.memorize_to}")
    ...
    try:
        ...
        profile_task = asyncio.create_task(
            core_memory_repo.find_by_user_ids(user_id_list),
            name="read_profiles"
        )
        
        doc_episodes, doc_profiles = await asyncio.gather(
            episode_task, profile_task
        )
    ...
    for doc_memcell in doc_memcells:
        if doc_memcell.group_id:
            ...
            original_data_list = _convert_original_data_for_profile_extractor(doc_memcell)
            ...
            business_memcell = MemCell(
                ...
                timestamp=doc_memcell.timestamp if hasattr(doc_memcell.timestamp, 'isoformat') else doc_memcell.timestamp,
                summary=getattr(doc_memcell, 'summary', ''),
                group_id=doc_memcell.group_id,
                participants=getattr(doc_memcell, 'participants', [doc_memcell.user_id] if doc_memcell.user_id else []),
                type=_convert_data_type_to_raw_data_type(getattr(doc_memcell, 'type', 'Conversation')),
    ...
    for doc_episode in doc_episodes:
        ...
        if (doc_episode.group_id and 
            episode_time >= request.memorize_from and 
            episode_time <= request.memorize_to):
            # 转换为Memory对象
            episode_memory = Memory(
                memory_type=MemoryType.EPISODE_SUMMARY,
                user_id=doc_episode.user_id,
                timestamp=doc_episode.timestamp if hasattr(doc_episode.timestamp, 'isoformat') else doc_episode.timestamp,
                ori_event_id_list=[],
                ...
                type=_convert_data_type_to_raw_data_type(getattr(doc_episode, 'type', 'Conversation')),
    ...
    for group_id, user_ids in group_user_mapping.items():
        logger.info(f"  - {group_id}: {len(user_ids)} 个用户 {list(user_ids)[:3]}{'...' if len(user_ids) > 3 else ''}")
        ...
        for group_id, user_ids in group_user_mapping.items():
            if profile_user_id in user_ids:
                ...
                profile_memory = ProfileMemory(
                    ...
                    keywords=[],
                    linked_entities=[],
                    extend={"core_memory_data": doc_profile}, # 将原始CoreMemory数据存在extend中
                    ...
                    projects_participated=_convert_projects_participated_list(doc_profile.projects_participated),
    ...
    # 统计分组结果
    all_groups = set(list(memcells_by_group.keys()) + list(episodes_by_group.keys()) + list(profiles_by_group.keys()))
    ...
    for group_id in all_groups:
        ...
        logger.info(f"  - {group_id}: MemCell={memcell_count}, Episode={episode_count}, Profile={profile_count}")
    ...
    # 定义带LLM并发限制的群组处理函数
    async def memorize_offline_by_group_id_with_limit(group_id: str, memcells: List[MemCell], old_memory_list: List[Memory]) -> List[Memory]:
        async with _global_llm_semaphore:
            return await memorize_offline_by_group_id(group_id, memcells, old_memory_list)

    # 阶段1: 按批次处理群组，提取 GROUP_PROFILE 和 PROFILE 记忆
    # 批次大小由 GROUP_BATCH_SIZE 控制，LLM并发由 Semaphore 控制
    logger.info(f"[memorize_offline] 开始阶段1: 群组记忆提取，共 {len(all_group_ids)} 个群组，批次大小 {GROUP_BATCH_SIZE}，LLM并发度 {LLM_CONCURRENCY_LIMIT}")
    for group_batch in _batched(all_group_ids, GROUP_BATCH_SIZE):
        ...
        for group_id in group_batch:
            ...
            doc_group_user_profiles = await group_user_profile_memory_repo.get_by_group_id(group_id)
            group_memory_list = []
            for doc_profile in doc_group_user_profiles:
                group_profile = ProfileMemory(
                    ...
                    interests=doc_profile.interests,
                    tendency=doc_profile.tendency,
                    group_importance_evidence=_convert_document_to_group_importance_evidence(doc_profile.group_importance_evidence)
                )
                group_memory_list.append(group_profile)

            # 读取群组画像原始数据
            doc_group_profile_raw = await group_profile_raw_repo.get_by_group_id(group_id)
            user_group_memory_list = []
            if doc_group_profile_raw:
                converted_fields = _convert_group_profile_raw_to_memory_format(doc_group_profile_raw)
                group_profile_raw = GroupProfileMemory(
                    ...
                    participants=[],
                    subject=doc_group_profile_raw.subject if hasattr(doc_group_profile_raw, 'subject') else "用户群组档案记忆",
                    summary=doc_group_profile_raw.summary if hasattr(doc_group_profile_raw, 'summary') else f"用户在群 {group_id} 的档案信息",
            ...
            old_memory_list = episodes + core_memory_list + group_memory_list + user_group_memory_list

            if memcells:
                # 使用带LLM并发限制的函数
                task = asyncio.create_task(
                    memorize_offline_by_group_id_with_limit(group_id, memcells, old_memory_list),
                    name=f"process_group_{group_id}"
                )
                group_tasks.append(task)
                task_names.append(task.get_name())
                logger.info(f"[memorize_offline] 创建群组任务: {group_id}, MemCell={len(memcells)}, 历史记忆={len(old_memory_list)}")
            else:
                logger.info(f"[memorize_offline] 群组 {group_id} 没有MemCell数据，跳过处理")
        ...
        for i, result in enumerate(group_results):
            ...
            task_name = task_names[i] if i < len(task_names) else "unknown_task"
            if isinstance(result, Exception):
                logger.error(f"[memorize_offline] 群组任务 {task_name} 执行失败: {result}")
            elif isinstance(result, list):
                batch_memories.extend(result)
                batch_memories.extend(result)
            else:
                logger.warning(f"[memorize_offline] 群组任务 {task_name} 返回未知格式: {type(result)}")

        if batch_memories:
            logger.info(f"[memorize_offline] 本批次提取 {len(batch_memories)} 个新记忆，准备保存...")
            try:
    ...
    if all_extracted_memories:
        profile_memories = [m for m in all_extracted_memories if m.memory_type == MemoryType.PROFILE]
        ...
        if user_ids:
            ...
            logger.info(f"[memorize_offline] 准备合并 {len(user_ids)} 个用户的 profile，其中 {len(core_profile_map)} 个有 core memory")
            ...
            async def merge_profile_for_user_with_limit(user_id: str):
                ...
                async with _global_llm_semaphore:
                    group_profile_memories = [m for m in profile_memories if m.user_id == user_id]
            ...
            for batch_idx, user_batch in enumerate(_batched(user_ids, user_batch_size), 1):
                logger.info(f"[memorize_offline] 处理用户批次 {batch_idx}/{(len(user_ids) + user_batch_size - 1) // user_batch_size}，本批次 {len(user_batch)} 个用户")
                ...
                for i, result in enumerate(user_results):
                    if isinstance(result, Exception):
                        logger.error(f"[memorize_offline] 用户 {user_batch[i]} 的profile合并失败: {result}")
                    else:
                ...
                if valid_user_profiles:
                    try:
                        ...
                        logger.info(f"[memorize_offline] 本批次 {len(valid_user_profiles)} 个用户profile保存成功")
                        valid_user_profiles_total.extend(valid_user_profiles)
                    except Exception as e:
                        logger.error(f"[memorize_offline] 保存本批次用户profile失败: {e}")
            
            logger.info(f"[memorize_offline] 阶段2完成，共合并并保存 {len(valid_user_profiles_total)} 个用户profile")

    logger.info(f"[memorize_offline] 离线记忆处理完成，共处理 {len(all_groups)} 个群组，提取 {len(all_extracted_memories)} 个新记忆")
