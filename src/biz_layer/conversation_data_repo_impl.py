...
class ConversationDataRepositoryImpl(ConversationDataRepository):
    ...
    async def save_conversation_data(self, raw_data_list: List[RawData], group_id: str) -> bool:
        ...
        logger.info("开始保存对话数据到Redis: group_id=%s, 数量=%d", group_id, len(raw_data_list))
        ...
        try:
            ...
            for raw_data in raw_data_list:
                try:
                    ...
                    if success:
                        saved_count += 1
                        logger.debug("保存RawData到Redis成功: data_id=%s", raw_data.data_id)
                    else:
                        logger.error("保存RawData到Redis失败: data_id=%s", raw_data.data_id)
            ...
            logger.info("对话数据保存到Redis完成: group_id=%s, 成功保存=%d/%d", 
                       group_id, saved_count, len(raw_data_list))
            return saved_count > 0
            
        except (RuntimeError, ConnectionError, TimeoutError, ValueError, TypeError) as e:
    ...
    async def get_conversation_data(
        ...
        Returns:
            List[RawData]: 对话数据列表
        """
        logger.info("开始从Redis获取对话数据: group_id=%s, start_time=%s, end_time=%s, limit=%d", 
        ...
        try:
            ...
            logger.info("从Redis获取对话数据完成: group_id=%s, 返回%d条数据", group_id, len(raw_data_list))
