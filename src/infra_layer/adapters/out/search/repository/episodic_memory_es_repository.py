    ...
    async def multi_search(
        ...
        try:
            ...
            if query:
                ...
                for word, word_score in sorted_query_with_scores:
                    should_queries.append(
                        Q("match", search_content__original={"query": word, "boost": word_score})
                    )
            ...
            if explain and query:
                ...
                logger.debug("✅ 情景记忆DSL多词搜索成功(explain模式): query=%s, user_id=%s, 找到 %d 条结果", 
                           search.to_dict(), user_id, len(hits))
            else:
                ...
                logger.debug("✅ 情景记忆DSL多词搜索成功: query=%s, user_id=%s, 找到 %d 条结果", 
                           search.to_dict(), user_id, len(hits))
            ...
            logger.error("❌ 情景记忆DSL多词搜索失败: query=%s, user_id=%s, error=%s", query, user_id, e)
            raise
        except Exception as e:
            logger.error("❌ 情景记忆DSL多词搜索失败（未知错误）: query=%s, user_id=%s, error=%s", query, user_id, e)
            raise
    ...
    async def delete_by_event_id(self, event_id: str, refresh: bool = False) -> bool:
        ...
        try:
            ...
            if result:
                ...
                logger.warning("⚠️ 根据event_id删除情景记忆失败，文档不存在: event_id=%s", event_id)
    ...
    async def delete_by_filters(
        ...
        try:
            ...
            if not filter_queries:
                raise ValueError("至少需要提供一个过滤条件（user_id、group_id或date_range）")
            ...
            logger.debug("✅ 根据过滤条件批量删除情景记忆成功: user_id=%s, group_id=%s, 删除了 %d 条记录", 
            ...
            logger.error("❌ 根据过滤条件批量删除情景记忆失败: user_id=%s, group_id=%s, error=%s", 
                        user_id, group_id, e)
            raise
