    ...
    def from_mongo(cls, source_doc: MongoPersonalSemanticMemory) -> EpisodicMemoryDoc:
        ...
        try:
            ...
            es_doc = EpisodicMemoryDoc(
                ...
                memcell_event_id_list=[source_doc.parent_episode_id]
            ...
            logger.error(
                "从 MongoDB PersonalSemanticMemory 文档转换为 ES 文档失败: %s", e
            )
            raise

    @classmethod
    def _build_search_content(
        cls, source_doc: MongoPersonalSemanticMemory
    ) -> List[str]:
        """
        构建搜索内容列表
