...
class ConversationStatusRawRepository(BaseRepository[ConversationStatus]):
    ...
    async def upsert_by_group_id(
        ...
        try:
            ...
            if existing_doc:
                ...
                print(f"[ConversationStatusRawRepository] 更新现有对话状态成功: {existing_doc}")
                return existing_doc
            ...
            try:
                ...
                print(f"[ConversationStatusRawRepository] 创建新对话状态成功: {new_doc}")
                return new_doc
                ...
                if "E11000" in error_str and "duplicate key" in error_str:
                    logger.warning("⚠️  并发创建冲突，重新查找并更新: group_id=%s", group_id)
                    ...
                    if retry_doc:
                        ...
                        print(f"[ConversationStatusRawRepository] 并发冲突后更新成功: {retry_doc}")
                        return retry_doc
                    else:
                        logger.error("❌ 并发冲突后仍无法找到记录: group_id=%s", group_id)
                        return None
                else:
                    # 其他类型的创建错误，直接抛出
                    raise create_error
    ...
    async def count_by_group_id(
        ...
        try:
            ...
            logger.debug("✅ 统计对话状态数量成功: group_id=%s, count=%d", group_id, count)
            return count
        except Exception as e:
