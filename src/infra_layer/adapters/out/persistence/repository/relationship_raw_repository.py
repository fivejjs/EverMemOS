...
class RelationshipRawRepository(BaseRepository[Relationship]):
    ...
    async def get_by_entity_ids(
        ...
        try:
            ...
            if result:
                logger.debug("✅ 根据实体ID获取关系成功: %s -> %s", source_entity_id, target_entity_id)
            else:
                logger.debug("⚠️  未找到关系: source=%s, target=%s", source_entity_id, target_entity_id)
            return result
    ...
    async def get_by_source_entity(
        ...
        try:
            ...
            logger.debug("✅ 根据源实体ID获取关系成功: %s, count=%d", source_entity_id, len(results))
            return results
        except Exception as e:
    ...
    async def get_by_target_entity(
        ...
        try:
            ...
            logger.debug("✅ 根据目标实体ID获取关系成功: %s, count=%d", target_entity_id, len(results))
            return results
        except Exception as e:
    ...
    async def update_by_entity_ids(
        ...
        try:
            ...
            if not existing_doc:
                logger.warning("⚠️  未找到要更新的关系: source=%s, target=%s", source_entity_id, target_entity_id)
                return None
            
            for key, value in update_data.items():
                setattr(existing_doc, key, value)
            await existing_doc.save(session=session)
            logger.debug("✅ 根据实体ID更新关系成功: %s -> %s", source_entity_id, target_entity_id)
            return existing_doc
        except Exception as e:
    ...
    async def delete_by_entity_ids(
        ...
        try:
            ...
            if not result:
                logger.warning("⚠️  未找到要删除的关系: source=%s, target=%s", source_entity_id, target_entity_id)
                return False
            
            await result.delete(session=session)
            logger.info("✅ 根据实体ID删除关系成功: %s -> %s", source_entity_id, target_entity_id)
            return True
        except Exception as e:
    ...
    async def get_relationships_by_entity(
        ...
        try:
            ...
            logger.debug("✅ 获取实体相关关系成功: entity=%s, count=%d", entity_id, len(results))
            return results
        except Exception as e:
    ...
    async def count_by_entity(
        ...
        try:
            ...
            logger.debug("✅ 统计实体关系数量成功: entity=%s, count=%d", entity_id, count)
            return count
        except Exception as e:
