...
class BehaviorHistoryRawRepository(BaseRepository[BehaviorHistory]):
    ...
    async def get_by_user_id(
        ...
        try:
            ...
            logger.debug("✅ 根据用户ID获取行为历史成功: %s, count=%d", user_id, len(results))
            return results
    ...
    async def get_by_time_range(
        ...
        try:
            ...
            logger.debug("✅ 根据时间范围获取行为历史成功: start=%d, end=%d, count=%d", 
                       start_timestamp, end_timestamp, len(results))
            return results
        except Exception as e:
    ...
    async def append_behavior(
        ...
        try:
            ...
            logger.debug("✅ 追加行为历史成功: user_id=%s, timestamp=%s", 
                       behavior_history.user_id, behavior_history.timestamp)
    ...
    async def delete_by_user_and_timestamp(
        ...
        try:
            ...
            if not result:
                logger.warning("⚠️  未找到要删除的行为历史: user_id=%s, timestamp=%d", user_id, timestamp)
                return False
            
            await result.delete(session=session)
            logger.info("✅ 根据用户ID和时间戳删除行为历史成功: %s, %d", user_id, timestamp)
            return True
        except Exception as e:
    ...
    async def get_recent_behaviors(
        ...
        try:
            ...
            logger.debug("✅ 获取用户最近行为历史成功: user_id=%s, hours=%d, count=%d", 
                       user_id, hours, len(results))
            return results
        except Exception as e:
    ...
    async def count_by_user(
        ...
        try:
            ...
            logger.debug("✅ 统计用户行为历史数量成功: user_id=%s, count=%d", user_id, count)
            return count
        except Exception as e:
    ...
    async def count_by_time_range(
        ...
        try:
            ...
            logger.debug("✅ 统计时间范围内行为历史数量成功: start=%d, end=%d, count=%d", 
                       start_timestamp, end_timestamp, count)
            return count
        except Exception as e:
