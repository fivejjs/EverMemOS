...
class CoreMemoryRawRepository(BaseRepository[CoreMemory]):
    ...
    async def ensure_latest(
        ...
        try:
            ...
            if latest_version.is_latest != True:
                ...
                logger.debug("✅ 设置最新版本标记: user_id=%s, version=%s", user_id, latest_version.version)
    ...
    async def get_by_user_id(
        ...
        try:
            ...
            if version_range is None:
                ...
                if result:
                    logger.debug("✅ 根据用户ID获取核心记忆成功: %s, version=%s", user_id, result.version)
                else:
                ...
                results = await self.model.find(
                    query_filter,
                    sort=[("version", -1)],  # 按版本倒序
                    session=session
                ).to_list()
                logger.debug("✅ 根据用户ID获取核心记忆版本成功: %s, version_range=%s, 找到 %d 条记录", 
                            user_id, version_range, len(results))
                return results
        except Exception as e:
    ...
    async def update_by_user_id(
        ...
        try:
            ...
            if not existing_doc:
                logger.warning("⚠️  未找到要更新的核心记忆: user_id=%s, version=%s", user_id, version)
                return None
            ...
            logger.debug("✅ 根据用户ID更新核心记忆成功: user_id=%s, version=%s", user_id, existing_doc.version)
    ...
    async def delete_by_user_id(
        ...
        try:
            ...
            if version is not None:
                ...
                if success:
                    logger.debug("✅ 根据用户ID和版本删除核心记忆成功: user_id=%s, version=%s", user_id, version)
                    # 删除后确保最新版本标记正确
                    await self.ensure_latest(user_id, session)
                else:
                    logger.warning("⚠️  未找到要删除的核心记忆: user_id=%s, version=%s", user_id, version)
            else:
                # 删除所有版本
                result = await self.model.find(query_filter, session=session).delete()
                deleted_count = result.deleted_count if hasattr(result, 'deleted_count') else 0
                ...
                if success:
                    logger.debug("✅ 根据用户ID删除所有核心记忆成功: user_id=%s, 删除 %d 条", user_id, deleted_count)
                else:
            ...
            if existing_doc:
                ...
                for key, value in update_data.items():
                    if hasattr(existing_doc, key):
                        setattr(existing_doc, key, value)
                await existing_doc.save(session=session)
                logger.debug("✅ 更新现有核心记忆成功: user_id=%s, version=%s", user_id, existing_doc.version)
                ...
                # 创建新记录时必须提供version
                if version is None:
                    logger.error("❌ 创建新核心记忆时必须提供version字段: user_id=%s", user_id)
                    raise ValueError(f"创建新核心记忆时必须提供version字段: user_id={user_id}")
                ...
                logger.info("✅ 创建新核心记忆成功: user_id=%s, version=%s", user_id, new_doc.version)
    ...
    async def find_by_user_ids(
        ...
        try:
            ...
            logger.debug("✅ 根据用户ID列表获取核心记忆成功: %d 个用户ID, only_latest=%s, 找到 %d 条记录", 
                        len(user_ids), only_latest, len(results))
            return results
        except Exception as e:
