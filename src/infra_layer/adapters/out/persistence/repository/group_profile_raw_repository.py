...
class GroupProfileRawRepository(BaseRepository[GroupProfile]):
    ...
    async def ensure_latest(
        ...
        try:
            ...
            if latest_version.is_latest != True:
                ...
                logger.debug("✅ 设置最新版本标记: group_id=%s, version=%s", group_id, latest_version.version)
    ...
    async def get_by_group_id(
        ...
        try:
            ...
            if result:
                logger.debug("✅ 根据群组ID获取群组记忆成功: %s, version=%s", group_id, result.version)
            else:
    ...
    async def update_by_group_id(
        ...
        try:
            ...
            if not existing_doc:
                logger.warning("⚠️  未找到要更新的群组记忆: group_id=%s, version=%s", group_id, version)
                return None
            ...
            logger.debug("✅ 根据群组ID更新群组记忆成功: group_id=%s, version=%s", group_id, existing_doc.version)
    ...
    async def delete_by_group_id(
        ...
        try:
            ...
            if version is not None:
                ...
                if not result:
                    logger.warning("⚠️  未找到要删除的群组记忆: group_id=%s, version=%s", group_id, version)
                    return False
                
                await result.delete(session=session)
                logger.debug("✅ 根据群组ID和版本删除群组记忆成功: group_id=%s, version=%s", group_id, version)
                ...
                if success:
                    logger.debug("✅ 根据群组ID删除所有群组记忆成功: group_id=%s, 删除 %d 条", group_id, deleted_count)
                else:
            ...
            if existing_doc:
                ...
                for key, value in update_data.items():
                    if hasattr(existing_doc, key):
                        setattr(existing_doc, key, value)
                await existing_doc.save(session=session)
                logger.debug("✅ 更新现有群组记忆成功: group_id=%s, version=%s", group_id, existing_doc.version)
                ...
                # 创建新记录时必须提供version
                if version is None:
                    logger.error("❌ 创建新群组记忆时必须提供version字段: group_id=%s", group_id)
                    raise ValueError(f"创建新群组记忆时必须提供version字段: group_id={group_id}")
                ...
                logger.info("✅ 创建新群组记忆成功: group_id=%s, version=%s", group_id, new_doc.version)
    ...
    async def find_by_group_ids(
        ...
        try:
            ...
            logger.debug("✅ 根据群组ID列表获取群组记忆成功: %d 个群组ID, only_latest=%s, 找到 %d 条记录", 
                        len(group_ids), only_latest, len(results))
            return results
        except Exception as e:
