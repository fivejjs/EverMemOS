...
class GroupUserProfileMemoryRawRepository(BaseRepository[GroupUserProfileMemory]):
    ...
    async def ensure_latest(
        ...
        try:
            ...
            if not latest_version:
                logger.debug("ℹ️  未找到需要更新的群组用户档案: user_id=%s, group_id=%s", user_id, group_id)
                return True
            ...
            if latest_version.is_latest != True:
                ...
                logger.debug("✅ 设置最新版本标记: user_id=%s, group_id=%s, version=%s", 
                           user_id, group_id, latest_version.version)
            
            return True
        except Exception as e:
            logger.error("❌ 确保最新版本标记失败: user_id=%s, group_id=%s, error=%s", 
                        user_id, group_id, e)
            return False
    ...
    async def get_by_user_group(
        ...
        try:
            ...
            if result:
                logger.debug("✅ 根据用户ID和群组ID获取群组用户档案成功: user_id=%s, group_id=%s, version=%s", 
                           user_id, group_id, result.version)
            else:
                logger.debug("ℹ️ 未找到群组用户档案: user_id=%s, group_id=%s", user_id, group_id)
            return result
        except Exception as e:
    ...
    async def update_by_user_group(
        ...
        try:
            ...
            if not existing_doc:
                logger.warning("⚠️ 未找到要更新的群组用户档案: user_id=%s, group_id=%s, version=%s", 
            ...
            logger.debug("✅ 根据用户ID和群组ID更新群组用户档案成功: user_id=%s, group_id=%s, version=%s", 
                       user_id, group_id, existing_doc.version)
    ...
    async def delete_by_user_group(
        ...
        try:
            ...
            if version is not None:
                ...
                if success:
                    logger.debug("✅ 根据用户ID、群组ID和版本删除群组用户档案成功: user_id=%s, group_id=%s, version=%s", 
                               user_id, group_id, version)
                    # 删除后确保最新版本标记正确
                    await self.ensure_latest(user_id, group_id, session)
                else:
                    logger.warning("⚠️ 未找到要删除的群组用户档案: user_id=%s, group_id=%s, version=%s", 
                                 user_id, group_id, version)
            else:
                # 删除所有版本
                result = await self.model.find(query_filter, session=session).delete()
                ...
                if success:
                    logger.debug("✅ 根据用户ID和群组ID删除所有群组用户档案成功: user_id=%s, group_id=%s, 删除 %d 条", 
                               user_id, group_id, deleted_count)
                else:
                    logger.warning("⚠️ 未找到要删除的群组用户档案: user_id=%s, group_id=%s", user_id, group_id)
            ...
            if existing_doc:
                ...
                for key, value in update_data.items():
                    if hasattr(existing_doc, key):
                        setattr(existing_doc, key, value)
                await existing_doc.save(session=session)
                logger.debug("✅ 更新现有群组用户档案成功: user_id=%s, group_id=%s, version=%s", 
                           user_id, group_id, existing_doc.version)
                ...
                # 创建新记录时必须提供version
                if version is None:
                    logger.error("❌ 创建新群组用户档案时必须提供version字段: user_id=%s, group_id=%s", 
                               user_id, group_id)
                    raise ValueError(f"创建新群组用户档案时必须提供version字段: user_id={user_id}, group_id={group_id}")
                ...
                logger.info("✅ 创建新群组用户档案成功: user_id=%s, group_id=%s, version=%s", 
                          user_id, group_id, new_doc.version)
    ...
    async def get_by_user_id(
        ...
        try:
            ...
            logger.debug("✅ 根据用户ID获取群组用户档案成功: user_id=%s, only_latest=%s, 找到 %d 条记录", 
                       user_id, only_latest, len(results))
            return results
        except Exception as e:
    ...
    async def get_by_group_id(
        ...
        try:
            ...
            logger.debug("✅ 根据群组ID获取群组用户档案成功: group_id=%s, only_latest=%s, 找到 %d 条记录", 
                       group_id, only_latest, len(results))
            return results
        except Exception as e:
    ...
    async def get_by_user_ids(
        ...
        try:
            ...
            logger.debug("✅ 根据用户ID列表获取群组用户档案成功: %d 个用户ID, group_id=%s, only_latest=%s, 找到 %d 条记录", 
                        len(user_ids), group_id, only_latest, len(results))
            return results
        except Exception as e:
    ...
    async def get_by_group_ids(
        ...
        try:
            ...
            logger.debug("✅ 根据群组ID列表获取群组用户档案成功: %d 个群组ID, user_id=%s, only_latest=%s, 找到 %d 条记录", 
                        len(group_ids), user_id, only_latest, len(results))
            return results
        except Exception as e:
    ...
    async def delete_by_user_id(
        ...
        try:
            ...
            deleted_count = result.deleted_count if hasattr(result, 'deleted_count') else 0
            logger.debug("✅ 根据用户ID删除群组用户档案成功: user_id=%s, 删除 %d 条记录", user_id, deleted_count)
    ...
    async def delete_by_group_id(
        ...
        try:
            ...
            deleted_count = result.deleted_count if hasattr(result, 'deleted_count') else 0
            logger.debug("✅ 根据群组ID删除群组用户档案成功: group_id=%s, 删除 %d 条记录", group_id, deleted_count)
            return deleted_count
        except Exception as e:
