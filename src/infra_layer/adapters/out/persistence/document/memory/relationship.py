...
class Relationship(DocumentBase, AuditBase):
    ...
    # 关系信息
    relationship: List[Dict[str, str]] = Field(..., description="关系列表，每个关系包含type、content、detail等字段")
    ...
    class Settings:
        ...
        name = "relationships"
        indexes = [
            IndexModel([("source_entity_id", ASCENDING), ("target_entity_id", ASCENDING)], 
                     unique=True, name="idx_source_target_unique"),
            IndexModel([("target_entity_id", ASCENDING), ("source_entity_id", ASCENDING)], 
                     unique=True, name="idx_target_source_unique"),
