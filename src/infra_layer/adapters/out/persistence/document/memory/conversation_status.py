...
class ConversationStatus(DocumentBase, AuditBase):
    ...
    old_msg_start_time: Optional[datetime] = Field(default=None, description="对话窗口读取起始时间")
    new_msg_start_time: Optional[datetime] = Field(default=None, description="累积新对话读取起始时间")
    last_memcell_time: Optional[datetime] = Field(default=None, description="累积memCell读取起始时间")
    ...
    class Settings:
        ...
        indexes = [
            ...
            IndexModel([("group_id", ASCENDING)], name="idx_group_id", unique=True),  # group_id 必须唯一
