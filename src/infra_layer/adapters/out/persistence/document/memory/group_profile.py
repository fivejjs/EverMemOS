...
class TopicInfo(BaseModel):
    ...
    id: Optional[str] = Field(default=None, description="话题唯一ID (系统生成，LLM不需要提供)")
    update_type: Optional[str] = Field(default=None, description="'new' | 'update' (仅用于增量更新时)")
    old_topic_id: Optional[str] = Field(default=None, description="更新时指向老topic (仅用于增量更新时)")
    evidences: Optional[List[str]] = Field(default_factory=list, description="memcell_ids 作为证据")
    confidence: Optional[str] = Field(default=None, description="'strong' | 'weak' - 置信度")
...
class RoleAssignment(BaseModel):
    ...
    user_id: str = Field(..., description="用户ID")
    user_name: str = Field(..., description="用户名")
    confidence: Optional[str] = Field(default=None, description="置信度: 'strong' | 'weak'")
    evidences: Optional[List[str]] = Field(default_factory=list, description="支持该角色分配的 memcell_ids")
...
class GroupProfile(DocumentBase, AuditBase):
    ...
    class Settings:
        ...
        indexes = [
            # 群组ID和版本联合唯一索引
            IndexModel([("group_id", ASCENDING), ("version", ASCENDING)], unique=True, name="idx_group_id_version_unique"),
            ...
            # 复合索引：群组ID + 更新时间
            IndexModel([("group_id", ASCENDING), ("updated_at", DESCENDING)], name="idx_group_id_updated_at")
        ]
