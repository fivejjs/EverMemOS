...
class UserProfile(DocumentBase, AuditBase):
    ...
    profile_data: Dict[str, Any] = Field(
        default_factory=dict, description="用户画像数据（包含角色、技能、偏好、性格等）"
    )

    # 元信息
    scenario: str = Field(
        default="group_chat", description="场景类型：group_chat 或 assistant"
    )
    confidence: float = Field(default=0.0, description="画像置信度 (0-1)")
    version: int = Field(default=1, description="画像版本号")

    # 聚类关联
    cluster_ids: List[str] = Field(default_factory=list, description="关联的聚类ID列表")
    memcell_count: int = Field(default=0, description="参与提取的 MemCell 数量")

    # 历史记录
    last_updated_cluster: Optional[str] = Field(
        default=None, description="最后一次更新时使用的聚类ID"
    )
