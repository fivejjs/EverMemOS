...
class BehaviorHistory(DocumentBase, AuditBase):
    ...
    # 行为信息
    behavior_type: List[str] = Field(..., description="行为类型列表（chat、follow-up、Smart-Reply、Vote、file、Email、link-doc等）")
    event_id: Optional[str] = Field(default=None, description="关联记忆单元ID（若存在）")
    meta: Optional[Dict[str, Any]] = Field(default=None, description="元信息：对话详情、Email原文等")
    ...
    class Settings:
        ...
        indexes = [
            IndexModel([("user_id", ASCENDING), ("behavior_type", ASCENDING), ("timestamp", ASCENDING)], 
