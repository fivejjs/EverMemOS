...
class ClusterState(DocumentBase, AuditBase):
    ...
    eventid_to_cluster: Dict[str, str] = Field(
        default_factory=dict, description="事件到聚类的映射"
    )
    ...
    cluster_centroids: Dict[str, List[float]] = Field(
        default_factory=dict, description="聚类中心向量 {cluster_id: vector}"
    )
    cluster_counts: Dict[str, int] = Field(
        default_factory=dict, description="聚类成员数量 {cluster_id: count}"
    )
    cluster_last_ts: Dict[str, Optional[float]] = Field(
        default_factory=dict, description="聚类最后时间戳 {cluster_id: timestamp}"
    )
