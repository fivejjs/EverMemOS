...
class Pipeline:
    ...
    async def run(
        ...
        if smoke_test:
            ...
            self.console.print(
                f"[yellow]   - Conversation: {dataset.conversations[0].conversation_id}[/yellow]"
            )
            self.console.print(
                f"[yellow]   - Messages: {len(dataset.conversations[0].messages)}[/yellow]"
            )
            self.console.print(
                f"[yellow]   - Questions: {len(dataset.qa_pairs)}[/yellow]\n"
            )
        ...
        if "evaluate" in stages and "evaluate" not in self.completed_stages:
            ...
            if self.checkpoint:
                self.checkpoint.save_checkpoint(
                    ...
                    answer_results=answer_results_data
                    if answer_results_data
    ...
    def _apply_smoke_test(
        ...
        self.logger.info(
            f"Smoke test: Conv {conv_id} - {msg_desc} messages, {qa_desc} questions"
        )
        ...
        return Dataset(
            ...
            metadata={
                **dataset.metadata,
                ...
                if num_messages > 0
                else len(first_conv.messages),
                "smoke_questions": num_questions
                if num_questions > 0
                else len(selected_qa_pairs),
            },
        )
