    ...
    async def add(
        ...
        **kwargs,
        ...
        if len(pending_conversations) == 0:
            ...
            console.print(
                f"   ✅ 成功处理: {successful_convs}/{len(pending_conversations)}",
                style="green",
            )
            ...
            if len(pending_conversations) > 0:
                console.print(
                    f"   🚀 平均每会话: {elapsed / len(pending_conversations):.2f} 秒",
                    style="cyan",
                )
        ...
        console.print(
            f"   BM25 索引: 需要构建 {bm25_to_build}, 已存在 {total_convs - bm25_to_build}"
        )
        if use_hybrid:
            console.print(
                f"   Embedding 索引: 需要构建 {emb_to_build}, 已存在 {total_convs - emb_to_build}"
            )
        ...
        if use_hybrid:
            if emb_to_build > 0:
                console.print(
                    f"\n🔨 构建 Embedding 索引 ({emb_to_build} 个会话)...",
                    style="yellow",
                )
