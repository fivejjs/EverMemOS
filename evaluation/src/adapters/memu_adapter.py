...
class MemuAdapter(OnlineAPIAdapter):
    ...
    async def add(self, conversations: List[Conversation], **kwargs) -> Dict[str, Any]:
        ...
        for conv in conversations:
            ...
            self.console.print(
                f"   Speaker A: {speaker_a} ({speaker_a_user_id})", style="dim"
            )
            self.console.print(
                f"   Speaker B: {speaker_b} ({speaker_b_user_id})", style="dim"
            )
            self.console.print(
                f"   Dual Perspective: {need_dual_perspective}", style="dim"
            )
        ...
        if task_ids:
            self.console.print(
                f"\n⏳ Waiting for {len(task_ids)} task(s) to complete...",
                style="bold yellow",
            )
    ...
    async def _add_single_user(
        ...
        base_messages = self._conversation_to_messages(
            conv, format_type="basic", perspective=perspective
        )
        ...
        self.console.print(
            f"   📤 Adding for {user_name} ({user_id}): {len(conversation_messages)} messages",
            style="dim",
        )
        ...
        while time.time() - start_time < self.task_timeout:
            ...
            for task_id in list(pending_tasks):
                try:
                    ...
                    self.console.print(
                        f"   ⚠️  Error checking task {task_id}: {e}", style="yellow"
                    )
            ...
            if not pending_tasks:
                self.console.print(
                    f"   ✅ All {total_tasks} tasks completed!", style="bold green"
                )
    ...
    async def _search_single_perspective(
        ...
        try:
            ...
            return SearchResult(
                ...
                results=[],
                retrieval_metadata={"error": str(e), "user_ids": [user_id]},
            )
