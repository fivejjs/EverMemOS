...
class Mem0Adapter(OnlineAPIAdapter):
    ...
    async def add(self, conversations: List[Conversation], **kwargs) -> Dict[str, Any]:
        ...
        self.console.print(f"\n{'=' * 60}", style="bold cyan")
        self.console.print(
            f"Stage 1: Adding to Mem0 (Dual Perspective)", style="bold cyan"
        )
        ...
        for conv in conversations:
            ...
            if conv.messages and conv.messages[0].timestamp:
                ...
                is_fake_timestamp = conv.messages[0].metadata.get(
                    "is_fake_timestamp", False
                )
            ...
            if is_fake_timestamp:
                self.console.print(
                    f"   ⚠️  Using fake timestamp (original data has no timestamp)",
                    style="yellow",
                )
            ...
            if need_dual_perspective:
                ...
                await self._add_dual_perspective(
                    conv,
                    speaker_a,
                    speaker_b,
                    speaker_a_user_id,
    ...
    async def _add_single_perspective(
        self, conv: Conversation, user_id: str, timestamp: int
    ):
        ...
        for msg in conv.messages:
            ...
            role = (
                "user" if msg.speaker_name.lower().startswith("user") else "assistant"
            )
    ...
    async def _add_dual_perspective(
        ...
        self.console.print(
            f"   Speaker A: {speaker_a} (user_id: {speaker_a_user_id})", style="dim"
        )
        self.console.print(
            f"   Speaker A Messages: {len(speaker_a_messages)}", style="dim"
        )
        self.console.print(
            f"   Speaker B: {speaker_b} (user_id: {speaker_b_user_id})", style="dim"
        )
        self.console.print(
            f"   Speaker B Messages: {len(speaker_b_messages)}", style="dim"
        )
        if truncated_count > 0:
            self.console.print(
                f"   ⚠️  Truncated {truncated_count} messages (>{self.max_content_length} chars)",
        ...
        await self._add_messages_for_user(
            speaker_a_messages, speaker_a_user_id, timestamp, f"Speaker A ({speaker_a})"
        )
        await self._add_messages_for_user(
            speaker_b_messages, speaker_b_user_id, timestamp, f"Speaker B ({speaker_b})"
        )

    async def _add_messages_for_user(
        self, messages: List[Dict], user_id: str, timestamp: int, description: str
    ):
        """
        为单个用户添加消息（带批量和重试）
    ...
    async def _search_single_perspective(
        ...
        try:
            ...
            self.console.print(f"  User ID: {user_id}", style="dim")
            self.console.print(
                f"  Results: {json.dumps(results, indent=2, ensure_ascii=False)}",
                style="dim",
            )
    ...
    async def _search_dual_perspective(
        ...
        try:
            ...
            self.console.print(
                f"    {json.dumps(search_speaker_a_results, indent=2, ensure_ascii=False)}",
                style="dim",
            )
            self.console.print(
                f"  Speaker B ({speaker_b}, user_id={speaker_b_user_id}):", style="dim"
            ...
            self.console.print(
                f"    {json.dumps(search_speaker_b_results, indent=2, ensure_ascii=False)}",
                style="dim",
            )
